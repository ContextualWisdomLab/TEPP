...
- The `n=16` cutoff is an implementation/resource boundary, not a scientific boundary. #491 evidence demonstrates an exact `n=17` counterexample and algebraically equivalent O(n) numerator. Resource envelopes are implementation facts, not scientific cutoffs: at **canonical aligned** coefficient diameter `D=2^53` after shared power-of-two normalization, the normalized O(n) distribution-independent intermediate envelope fits through `n=2_047`, whereas the exact pair-square numerator extremum fits through `n=4_095`. Production admission remains `n<=16` until release-mode CPU/allocation and applicable buyer-path p95 evidence establish a budget.
...
- The predecessor `D=2^58,n=65` refusal is invalid and superseded. After dividing the shared `2^58` unit, coefficients are zero/one, the O(n) intermediates are `4_160` and `4_096`, and the normalized numerator `64` restores exactly to `2^122`; the linear/hybrid path must admit without pair fallback.
...
- Accumulator-bound characterization `b7e4da353ac58069afd73ee7c0e8427d49993fdb` proves that after canonical minimum anchoring all coefficients are nonnegative integers with at least one zero, so `Σc_i <= Σc_i² <= Σ(i<j)(c_i-c_j)²`. Therefore any canonical pair numerator that fits `u128` already bounds both O(n) accumulators; searching for pair-admitted coefficient-sum or square-sum overflow is an invalid acceptance criterion. Remaining equivalence work belongs to full-width products/subtraction, dyadic restoration, denominator/midpoint proof, and represented-residual admission.
...
- Hybrid resource measurements must prove which path actually ran. RED `3136739460ef0c8e13c044a7e5b04891e4f4e23d` and repair `ce4ed2722e160eb0ca0ee2d636a5eca55e3ff2d5` extend the harness so it retains buffered O(n²), allocation-free two-pass O(n²), normalized narrow O(n), the wider-product O(n) reference, and the predecessor narrow→pair hybrid while adding `narrow O(n) -> Wide256 O(n) -> pairwise fail-closed fallback`. It requires restored-numerator equality before timing and records `geometry`, `kernel`, normalized `unit_exponent`, target pair-record layout/capacity/payload, `used_wide_product`, and `used_pairwise_fallback`. On odd `D=2^58+1,n=65`, the predecessor hybrid remains a pair-allocation comparison baseline while the new candidate must select Wide256 and avoid pair allocation.
...
Resource arithmetic remains separated from scientific admission. Exact pair records are 120 at `n=16`, 136 at `n=17`, 2,096,128 at `n=2,048`, and **4,997,541** at `n=3,162`. For canonical aligned diameter `D=2^53`, the normalized narrow O(n) checked intermediate envelope `n²D²` fits through `n=2,047`, but the exact pair-square numerator extremum `floor(n²/4)D²` fits through `n=4,095`. The unreduced scientific denominator `n²(n-1)` stays at or below `2^53` through `n=208,064` and exceeds it at `208,065`; because production uses the GCD-reduced denominator, none of these values is a universal refusal count.

RED `4f1bd2c343cf2d54905a07c257a570a89dc575d3` and repair `d423b57797b6f7f127e61e0679f9ee9841525c77` remove the representation-dependent raw `D=2^58,n=65` refusal by normalizing the common dyadic unit. Final narrow-reference hardening `96f17c02edba0792f61e0e92167703a6ae4e40d0` makes restored-scale overflow fail closed. The valid narrow strict-subset geometry is odd `D=2^58+1,n=65`: the pair numerator remains a 123-bit `u128`, while the normalized narrow O(n) products require 129 bits before cancellation.

Characterization `081000289f5a52e94863026d55696ee2a4daf923` adds a dependency-free two-limb `Wide256` reference for those wider products and checked cancellation. It recovers the odd-boundary exact pair numerator without changing the pair-reference scientific meaning.
