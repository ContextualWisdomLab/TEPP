...
| GAP-011 | TEPP is not yet an operable multi-tenant service or supported release. | `partial` | product vertical | `e65cd66` (library contracts only); durable tenant-isolated analysis-run requests, append-only state events, and engine-result artifact persistence are an unmerged candidate on #287; typed durable reads, retained-session locking, request/status rebinding, and atomic terminal publication are stacked on #288; tenant-bound manifest materialization is stacked on #289 | [#174](https://github.com/ContextualWisdomLab/TEPP/issues/174) / [#166](https://github.com/ContextualWisdomLab/TEPP/issues/166) / [PR #287](https://github.com/ContextualWisdomLab/TEPP/pull/287) / [PR #288](https://github.com/ContextualWisdomLab/TEPP/pull/288) / [PR #289](https://github.com/ContextualWisdomLab/TEPP/pull/289) | `152bd8bdd086` | Land #287, then revalidate #288 and #289 in dependency order with exact-head checks and independent review; executable workers, protected object ingestion, OIDC/purpose enforcement, OpenTelemetry/SLOs, load/recovery, signed release/SBOM/provenance, assurance evidence, and support policy remain. |
...
| [#289](https://github.com/ContextualWisdomLab/TEPP/pull/289) | Tenant-bound reproducibility-manifest materialization for #166 (GAP-011), stacked on #288 | Land #287 and #288 first, then rerun workflows on exact head `152bd8bdd086` against main and obtain qualifying independent approval. |
