...
| GAP-003A | Immutable evidence cannot yet be submitted to a durable validation run that produces operator-usable scientific acceptance evidence. | `partial` | active adapter integration | `e65cd66` (validation metrics are library-level only) | [#166](https://github.com/ContextualWisdomLab/TEPP/issues/166) / Proposed adapter vehicle [#469](https://github.com/ContextualWisdomLab/TEPP/pull/469) | `f81963d32884d1d081d82842e3c4f7a34b1e91a3` (pre-repair exact head) | #469 consolidates contextual-orchestrator interpretation-run retrieval, lookup, stored-request, and CLI adapters; it does not close durable execution. Exact-head CI found a missing public payload export and formatter drift. Compose/API execution must still bind immutable evidence, cutoffs, model configuration, validation metrics, and reproducibility manifests to one idempotent run. |
