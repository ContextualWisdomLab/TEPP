...
- Stacked PRs #284 and #285 add a topic-context posterior artifact and bind its
  identifiers to the fitted draw basis. Their parent branches must synchronize
  before child-head checks can become merge evidence.
- Snapshot facts were fetched live at 2026-08-28T04:34:26Z against protected
...
| Open pull requests | **8** | Active queue: #272 `TIPREDVARstd`, #279 interval consistency, #280 `discreteDRIFTstd`, #281 this baseline refresh, #282 fitted candidate-`K` selection, #283 ADR identity repair, #284 topic-context artifact assembly, and #285 draw-basis identity binding. |
...
The residual open PRs are new forward work (#272 and #279–#285), not the
...
2026-08-28T04:34:26Z against protected main `b03cc378`. Review decisions,
...
| #281 | `296a491a652121efcb2b0b71ae83662cd6c1a0f1` | false | main | docs(gap): refresh protected-main and eight-PR queue |
| #282 | `2858575ccc84fa900efd5f8512b7d685819dfad5` | false | main | feat(topic): execute fitted candidate-K selection |
| #283 | `b7580e85cf46e92b038039484343421f3e9e6e54` | false | main | fix(adr): enforce unique decision identities |
| #284 | `65b2c5f087eff9ba0ed8acd313e243569949898e` | false | feat/topic-lineage-candidate-k | feat(topic): assemble context posterior artifact |
| #285 | `3e2d795e7bd4440cf2ffcff292b878a9d6e152eb` | false | feat/topic-context-producer | fix(topic): bind artifact ids to fitted draw basis |
...
| GAP-004 | The central shared-latent temporal/relational topic estimator is absent. | `partial` | product vertical | CPU `f64` TRSL-TM reference estimator with ALR/ILR coordinates and refusal gates is implemented-main (v0.2.0 `topic_measurement`); fitted candidate-`K` execution and topic-context artifacts remain stacked open-branch candidates on #282/#284/#285 | [#167](https://github.com/ContextualWisdomLab/TEPP/issues/167) / [PR #282](https://github.com/ContextualWisdomLab/TEPP/pull/282) / [PR #284](https://github.com/ContextualWisdomLab/TEPP/pull/284) / [PR #285](https://github.com/ContextualWisdomLab/TEPP/pull/285) | `2858575ccc84` / `65b2c5f0a0a3` / `3e2d795e4653` | Synchronize the stack and land each PR only after its current-head checks and independent review; GPU, method effects, full Bayesian sampling, and topic birth/split/merge remain. This is not full #167 closure. |
...
|---:|---|---|
| [#272](https://github.com/ContextualWisdomLab/TEPP/pull/272) | Driver p.16 `TIPREDVARstd` restore (GAP-006 recovery family) | Resolve the current-head Strix failure before marking ready; independent approval remains required. |
| [#279](https://github.com/ContextualWisdomLab/TEPP/pull/279) | Bounded Allen/CHRONOS interval consistency (GAP-007) | Resolve the current-head Strix failure and obtain qualifying independent approval before merge. |
...
| [#284](https://github.com/ContextualWisdomLab/TEPP/pull/284) | Topic-context posterior artifact assembly stacked on #282 (GAP-004) | Synchronize with the moved #282 parent, repair its failing exact-head check, and obtain qualifying independent approval. |
| [#285](https://github.com/ContextualWisdomLab/TEPP/pull/285) | Fitted draw-basis identity binding stacked on #284 (GAP-004) | Synchronize both parent branches, complete exact-head checks, and obtain qualifying independent approval. |
## Delivery sequence
...
4. **Build the CPU topic estimator:** #167 — reference estimator landed; fitted candidate-`K` execution and topic-context artifacts are stacked open-branch candidates on #282/#284/#285, while full Bayesian fitting remains.
