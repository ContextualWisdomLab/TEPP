...
admission and transposition, over exactly the draw matrix passed to
...
- after cutoff/snapshot admission and draw-major transposition, compute
  `complete_data_draws_sha256` over the exact matrix sent to `psychometric_core`.
  The hash starts with the domain separator
  `tepp.rubin_loading.complete_data_draws.v1\0`, then admitted observation and
  draw counts as big-endian `u64`, then each finite draw value's exact IEEE-754
  bits as big-endian bytes, preserving draw order and admitted observation
  order. Future-unavailable rows are absent from both the matrix and the hash;
- bind that canonical lowercase SHA-256 into the Rubin artifact behind a
  ...
  noncanonical. The artifact's own SHA-256 therefore commits to the concrete
  admitted draw payload identity in addition to its statistical summaries;
...
activation authority contract, #515 closes concrete payload substitution, and
#516 makes the executor the canonical payload-hash owner. No positive artifact
...
factor-score/draw matrix, its canonical `complete_data_draws_sha256`, or its
...
shape, not values. The executor, not an arbitrary caller, owns the canonical
hash representation of the admitted draw-major matrix. The public receipt binds
that `complete_data_draws_sha256` and activation receives the executor-produced
runtime value independently. Receipt/runtime mismatch is rejected before
class-level approval. This prevents reusing a receipt for different draw values
with an identical shape; it does not certify that a matching payload actually
came from the approved generator.
...
19. Let each caller choose its own draw serialization before hashing — rejected
    because the same matrix could receive different digests and the same digest
    label could be detached from cutoff admission. `analysis_engine` hashes the
    exact admitted matrix with one domain-separated binary contract instead.
...
executor-produced concrete draw payload digest. Draft #506 can represent and
reject this authority boundary but cannot populate the production registry from
its own candidate evidence.
...
artifact now also carries an executor-produced identity for the exact admitted
draw matrix, so a later activation receipt need not depend on consumer-defined
...
missing concrete draw-payload commitment; #516 makes the executor/artifact the
canonical payload-digest owner.
...
Draft/Proposed and not implemented-main while #503, #505, #515/#516, and the
...
receipt/runtime draw-payload digest mismatch refusal, cutoff-safe executor draw-
payload digest stability, admitted-value sensitivity, and malformed artifact
draw-digest refusal.
...
Evidence snapshot identity/digest authority, executor-owned concrete draw-
payload identity, exact runtime design-point admission, resource admission,
claim-specific validation evidence, canonical analysis identity, and the Rubin-
versus-Mislevy / draw-generation activation boundaries.
