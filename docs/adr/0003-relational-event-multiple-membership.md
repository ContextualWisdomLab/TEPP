...
**Implementation maturity:** partial — membership network and event mention/instance separation implemented-main; typed relation graph with forward-only transitions implemented-main; causal-language identification identity in `causal_language` on the active PR; multilevel estimators and persistence remain accepted-target  
**Date:** 2026-08-05  
...
Observed relation evidence, inferred relations, and promoted transition edges remain distinct. Relation absence is not silently interpreted as evidence of no relationship. Association, temporal precedence, and document links remain distinct from identified causal claims.
