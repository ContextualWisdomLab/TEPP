...
**Implementation maturity:** partial — membership network and event mention/instance separation implemented-main; absence-is-not-negative identity in `relation_absence` on the active PR; typed relation graph with forward-only transitions implemented-main; multilevel estimators and persistence remain accepted-target  
**Date:** 2026-08-05  
