...
**Implementation maturity:** partial — membership network and event mention/instance separation implemented-main; typed target-kind identity in `membership_target` on the active PR; typed relation graph with forward-only transitions active-PR; multilevel estimators and persistence remain accepted-target  
**Date:** 2026-08-05  
