...
| [0009](0009-purpose-bound-pii-governance.md) | Purpose-bound PII governance without blanket masking | Accepted | partial | Privileged-access audit replay in `privileged_access` is active-PR; persistence retention/deletion/legal-hold and provider-payload minimization are implemented-main; deployment evidence remains accepted-target. |
