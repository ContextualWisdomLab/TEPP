...
| [0009](0009-purpose-bound-pii-governance.md) | Purpose-bound PII governance without blanket masking | Accepted | active-PR | `operational_log` `try_record` is the only recording API; `audit_event` inserts call it on the active PR; tenant/purpose/role/lifetime, live HTTP, and certification evidence remain accepted-target. |
