...
| [0010](0010-adaptive-llm-orchestration.md) | Adaptive LLM orchestration and test-time compute | Accepted | partial | `tepp_api` router/ablation/orchestrator binding and `orchestrator_live` loopback listener are active-PR; live NIM execution, TLS, and production ablation evidence remain accepted-target. |
| [0011](0011-standalone-modular-msa-boundary.md) | Standalone operation and modular CWL MSA boundary | Accepted | partial | Owns cross-service persistence/credential/API authority; naruon HTTPS interchange implemented-main and orchestrator loopback live HTTP/1.1 is active-PR; no direct cross-service application-table coupling. |
