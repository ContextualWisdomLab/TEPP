...
| [0009](0009-purpose-bound-pii-governance.md) | Purpose-bound PII governance without blanket masking | Accepted | active-PR | `encrypted_mapping` AES-256-GCM envelope on the active PR; retention/deletion/legal-hold remain active-PR while provider-payload minimization is implemented-main; persistence/KMS and remaining adapters stay accepted-target. Controls are not a certification claim. |
