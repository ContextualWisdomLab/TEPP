...
| [0002](0002-six-clock-temporal-semantics.md) | Six-clock temporal semantics and fail-closed historical leakage prevention | Accepted | active-PR | Membership availability cutoff in `membership_cutoff` on the active PR; remaining graph/split enforcement stays accepted-target. |
| [0003](0003-relational-event-multiple-membership.md) | Relational event ontology and time-varying cross-classified multiple membership | Accepted | active-PR | Cutoff-safe multiple-membership filtering in `membership_cutoff` on the active PR; full multilevel estimators remain accepted-target. |
...
| [0009](0009-purpose-bound-pii-governance.md) | Purpose-bound PII governance without blanket masking | Accepted | accepted-target | Controls are normative architecture; deployment/control evidence is not yet a certification claim. |
