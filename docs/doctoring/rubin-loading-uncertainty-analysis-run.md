...
Scientific-evidence review found two additional boundaries. First, the initial Monte Carlo harness generated a different synthetic population on every replication while reusing one supposedly immutable snapshot identity and one accepted run identity within each scenario. That made the numerical summaries reproducible but violated the product provenance semantics the acceptance test claimed to exercise end to end. Second, the Analysis Run still cannot identify which versioned generator/imputer produced arbitrary caller-supplied complete-data draws, so scoped recovery evidence cannot become universal inferential authorization.
...
- Scientific acceptance begins at `aa39f6d28d697d91014ab45daffa06f7c3182615`. The scientific-evidence child keeps the estimator boundary unchanged, centers the `T` interval diagnostic on Rubin `Q̄`, keeps robust `point_estimate_mean` as the separate recovery target, uses checked deterministic count conversion, and records attempted/recovered/failed denominators explicitly.
- Denominator wording repair `09bb1c8712d66fe41720ed4b4acd1838e50d5eab` states the executable rule exactly: numerical summaries are over recovered executions, while a passing scenario separately requires `recovered = attempted` and `failed = 0`.
- Claim-scope research update `45391afacf77412d4ff2b3d1ab47b8e776bbbf04` records that the checked coverage belongs to the declared draw generator/design rather than arbitrary finite draw matrices. ADR update `483a1806d6277095b1d749049fb32d8534e640a3` binds that projection rule to ADR 0014 and #505.
- Replicate-identity RED `16e3c151d6cadd79c1ec8e32d74a8401333be42c` demonstrates that the acceptance helper aliases distinct Monte Carlo populations to one immutable snapshot identity. Causal repair `ed2ac416ac42cb15909cefde0581af408de652e7` assigns every generated population its own snapshot identity, every Analysis Run execution its own idempotency/run identity, and keeps the three rolling-origin executions within one replication on the same immutable snapshot while using distinct run receipts.
...
`crates/analysis_engine/tests/rubin_loading_scientific_acceptance.rs` owns the executable repeated-sampling gate. Eight 512-replication scenarios vary observation count, draw count, true loading, and residual scale while exercising positive between-draw uncertainty. Attempted, recovered, and failed counts are recorded separately and the harness asserts `attempted = recovered + failed`. Bias, RMSE, coverage, and their Monte Carlo standard errors are computed over recovered executions; the scenario acceptance gate separately requires `recovered = attempted` and `failed = 0`, so no passing scientific result can silently drop a refused execution from those metric denominators.

Each generated Monte Carlo population is now modeled as a different immutable snapshot and a different Analysis Run. The numerical generator remains deterministic, but no snapshot or run receipt is silently reused across different generated datasets. In the rolling-origin contract, early-full, early-prefix, and later-cutoff runs use distinct run identities while sharing the same replication-specific snapshot because they are alternative historical views of one generated population.
...
The evidence still does not prove arbitrary draw-generation validity. Issue #505 owns versioned generator/analysis provenance plus the validation-evidence identity needed for inferential activation. Until that contract exists, the protected `Q̄/Ū/B/T` arithmetic may remain a bounded descriptive result while broader scientific/product claim promotion stays fail closed outside the explicitly validated design.
