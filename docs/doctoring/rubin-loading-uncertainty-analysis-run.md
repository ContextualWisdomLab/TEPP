...
- Scientific acceptance `aa39f6d28d697d91014ab45daffa06f7c3182615` adds deterministic repeated-sampling and rolling-origin evidence through the public Analysis Run profile. `docs/research/rubin-loading-uncertainty-scientific-acceptance.md` records the predeclared design, exact seeds, attempted/recovered/failed denominators, bias/RMSE and Monte Carlo uncertainty, Rubin `T` behavior, interval diagnostic boundary, and checked numerical evidence.
...
The point-estimate gate reports bias, RMSE, and their Monte Carlo uncertainty against known loading truth. The Rubin uncertainty gate records mean `Ū`, `B`, and `T`. The 95% interval check is explicitly a large-sample normal diagnostic `point_estimate_mean ± 1.959963984540054 * sqrt(T)`, not a claim that the profile implements Rubin small-sample degrees of freedom. Its empirical coverage is accepted only within the predeclared design and Monte Carlo envelope recorded in the research note.
