...
| HTTP analysis-run retry-lineage | `tepp_api` `naruon_analysis_run_retry_lineage_exchange` → `GET /v1/analysis-runs/{run_id}/retries` | naruon → TEPP |
| Typed retry-lineage CLI on spawned `tepp-loopback` TCP | `tepp_api` `tepp-retry-lineage` renders the typed GET onto the packaged loopback listener | naruon → TEPP |
...
| Live loopback retry-lineage GET | `tepp_api` `NaruonLiveService` → `GET /v1/analysis-runs/{run_id}/retries` (Naruon only; empty `retries` on accepted creates) | naruon → TEPP |
...
- scientific-metric keys (`rmse`, `bias`, `coverage`, `se_gate`, `scientific_acceptance`, `report`, `terminal_result`, `tenant_workspace_id`, `snapshot_id`) on a retry-lineage body → reject;
