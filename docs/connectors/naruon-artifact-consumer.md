...
| CLI analysis-run collection | `tepp_api` `tepp-analysis-runs list` → loopback `GET /v1/analysis-runs` | naruon → TEPP |
| CLI analysis-run cancel | `tepp_api` `tepp-analysis-runs cancel` → loopback `POST /v1/analysis-runs/{run_id}/cancel` | naruon → TEPP |
| CLI analysis-run create | `tepp_api` `tepp-analysis-runs create` → loopback `POST /v1/analysis-runs` | naruon → TEPP |
...
- scientific-metric keys (`rmse`, `bias`, `coverage`, `se_gate`, `scientific_acceptance`, `report`) on a create CLI body or accepted receipt → reject;
