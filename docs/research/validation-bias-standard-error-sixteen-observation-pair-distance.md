...
This is not a payload-specific branch and does not claim globally correctly rounded standard errors for arbitrary `n`. However, demonstrated one-ULP counterexamples now span every bounded sample size from `n=4` through `n=16`. At the time of repair `a509ae9e46c8ffc2cc3ef4f0e904774ad2516e1f`, treating the next integer sample count as the scientific boundary was recognized as an interim design: `n=17` remained on the established translated path while later work characterized the O(n²) cost and checked-integer proof budget. That predecessor cutoff has since been superseded. Current production first attempts the checked O(n) neutral-zero exact proof for every `n >= 3`; the O(n²) pairwise reference is used only after O(n) refusal at `n <= 16`, and an O(n) refusal above sixteen delegates directly to the established fallback without allocating the quadratic pairwise proof.
