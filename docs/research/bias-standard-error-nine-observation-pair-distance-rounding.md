# Nine-observation mean-bias standard error: exact pair-distance rounding

## Problem

After GAP-117, the bounded pair-distance/midpoint proof covered four through eight observations. A nine-observation sample with exact represented residuals still fell through to the translated floating second-moment path:

```text
truth     = [0, 0, 0, 0, 0, 0, 0, 0, 0]
recovered = [354161013, 513228884, 592175752, 1188877551, 1313358136,
             1582012649, 1600441891, 1764736057, 1957134661]
```

All values are exactly represented binary64 integers and all pairwise differences are exact. The thirty-six squared pair distances sum to

```text
N = 24907531253053169466.
```

For `n=9`,

```text
SE(mean)^2 = sum_{i<j}(r_i-r_j)^2 / (n^2(n-1)) = N / 648.
```

`gcd(N,648)=162`, giving the exact reduced radicand `153750192920081293/4`. Exact rational midpoint comparison gives the correctly rounded binary64 square-root bits `0x41a75f1f64895d36`; the predecessor translated floating moment/`sqrt` path returns adjacent lower `0x41a75f1f64895d35`. Reordering the same represented multiset and mirroring its sign preserve the same mismatch. This is deterministic represented-input arithmetic error, not Monte Carlo uncertainty.

## Decision

Public RED `c7b6537763f303069aa11e4ebfa12b7b3093448c` adds multiple permutations and sign mirrors for the nine-observation sample. Causal repair `bcd2ab5e1e6bd38f0661e7bb47e5d59a78830499` extends only the already bounded exact pair-distance path from `n=4..=8` to `n=4..=9`.
...
- Public executable contract: `crates/validation_core/tests/bias_standard_error_nine_observation_pair_distance_contract.rs`.
- RED: `c7b6537763f303069aa11e4ebfa12b7b3093448c`.
- Causal source repair: `bcd2ab5e1e6bd38f0661e7bb47e5d59a78830499`.
- CHANGELOG: `f575fa2143e0e46058dd62d6dcdd6e6c3e6354b1`.
