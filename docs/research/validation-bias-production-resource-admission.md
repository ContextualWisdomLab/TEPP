...
That constant belongs to the Analysis Run application boundary. `validation_core` does not currently consume a typed/versioned admission object that proves every production call to `bias_standard_error` came through `AnalysisCorpus`. Therefore the value `100_000` must not be copied into `validation_core`, treated as a public-library domain restriction, or used to delete numerical refusals.

The missing trace is architectural, not arithmetic: establish the supported buyer call path from an admitted Analysis Run population to the Validation Evidence computation, then keep the resource policy at its owner and pass only the admitted population/contract required by the numerical boundary.
...
This witness is not a realistic product fixture. Its observation count is about `1.8014398509481984e11` times the current Analysis Run in-memory cap. Materializing only the two public `f64` input slices would require
...
A buyer-bound proof may use the Analysis Run population limit only after the buyer call trace is explicit. If that trace proves all supported product executions have `n <= 100_000`, the implementation-matched forward-error obligation can be bounded on that finite admitted range. That would narrow product acceptance evidence; it would not silently redefine the source domain of the public validation library.
...
2. Add or identify the typed application contract that proves the population passed to the supported buyer Validation Evidence path was admitted under that rule.
3. Test the application boundary at the maximum admitted population and at the first refused population without allocating an artificial source-domain witness beyond the product contract.
4. Derive the floating translated-dispersion error bound against the exact implementation and the supported product population range. Include square rounding, deterministic sorted compensation, final correction addition, sample-count conversion, multiplication, and FMA.
...
- The helper false-zero branch is known reachable in the public source domain but presently outside realistic in-memory product materialization. Close its product acceptance gap through an owner-correct bounded representation/resource contract while preserving numerical fail-closed behavior.
- The translated nonpositive-dispersion branch has an exact-real positive margin but still needs an implementation-matched floating proof or a compact caller-valid counterexample. A verified Analysis Run population contract can narrow the buyer proof domain, but cannot substitute for that proof.

This separation keeps resource policy, numerical truth, and coverage evidence independently auditable.
