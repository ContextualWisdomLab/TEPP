...
5. `record_budget_ablation` requires a `direct` baseline and reports whether the compared budget is within a 10 percent relative band;
6. `bind_contextual_orchestrator` emits a credential-free execution binding and refuses abstention.
...
- ablation rejects a non-direct baseline and reports incomparable zero or wide-band budgets;
- orchestrator bindings omit credentials and refuse abstention.
