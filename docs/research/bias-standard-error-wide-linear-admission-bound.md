...
Do not widen production `validation_core::bias_standard_error` solely because the exact-rounding primitive now supports wider comparison products. Production admission remains `n=4..=16`. A budget change requires represented-input admission equivalence for the complete wider route, exact tie-to-even preservation, recorded Rust 1.98.0 `--release` raw CSV, CPU/OS/build metadata, allocator/RSS evidence, applicable full buyer-path p95 evidence, exact-head Rust/rustdoc/100% line+branch coverage/security/documentation GREEN, and qualifying independent current-head review.
