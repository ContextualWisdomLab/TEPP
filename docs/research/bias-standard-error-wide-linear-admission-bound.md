...
The product-width theorem makes a post-Wide256 pair fallback look redundant for the numerator identity within the canonical `u128` coefficient domain. The represented `n=2050` midpoint finding showed why this did not extend automatically to the whole exact-rounding proof. The exponent-safe comparison is now integrated into the authoritative rounding primitive, and `7a2ab0a1ef7a72d8cc9b9253d7f92c493e578943` now demonstrates equality of the pairwise and wide-linear exact-ratio inputs across a deterministic represented-input slice. The remaining equivalence work moves outward to production canonicalization/routing and fail-closed decisions across the represented domain, not to another sample-count staircase.
...
Do not widen production `validation_core::bias_standard_error` solely because the exact-rounding primitive now supports wider comparison products or because the deterministic represented route-equivalence slice passes. Production admission remains `n=4..=16`. A budget change requires represented-input admission equivalence for the complete production wider route, exact tie-to-even preservation, recorded Rust 1.98.0 `--release` raw CSV, CPU/OS/build metadata, allocator/RSS evidence, applicable full buyer-path p95 evidence, exact-head Rust/rustdoc/100% line+branch coverage/security/documentation GREEN, and qualifying independent current-head review.
