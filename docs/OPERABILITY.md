...
- side-by-side buffered O(n²), allocation-free two-pass O(n²), normalized narrow O(n), two-limb wider-product O(n), and hybrid evidence, with exact equality of restored pair-square numerators before timing;
- hybrid timing covering the common-power normalized admission and the odd-diameter admitted/refused boundary, with `unit_exponent` and `used_pairwise_fallback` recorded in the raw CSV;
...
- admitted/refused-set comparison between the existing pairwise proof and every O(n) candidate across sample count, canonical aligned coefficient diameter/exponent spread, and coefficient distribution; proof refusal must fall back rather than alter scientific meaning;
- evidence for where the bounded two-limb reference itself refuses because coefficient or square accumulation exceeds `u128`; success at one 129-bit product boundary is not a global proof;
