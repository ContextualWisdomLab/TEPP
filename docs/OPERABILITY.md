...
RED `3136739460ef0c8e13c044a7e5b04891e4f4e23d` makes the next resource route executable by requiring a `narrow O(n) -> Wide256 O(n) -> pairwise fail-closed fallback` kernel before it existed. Repair `ce4ed2722e160eb0ca0ee2d636a5eca55e3ff2d5` implements that candidate in `crates/validation_core/examples/bias_se_exact_proof_budget.rs` without changing production arithmetic. The predecessor narrow→buffered-pair hybrid remains in the same harness so the cost of unnecessary pair allocation is measurable rather than inferred.
