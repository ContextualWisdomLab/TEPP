...
| standalone + modular CWL MSA / no cross-service DB coupling | ADR 0011; `docs/API_CONTRACT.md`; `docs/research/production-tls-bind-gates.md` | current standalone crates; naruon loopback listener implemented-main; `service_tls` production bind gates on the active PR; remaining live listeners | partial |
| naruon modular artifact consumer boundary | ADR 0011/0012; API contract | `docs/connectors/naruon-artifact-consumer.md` + PR #22 versioned consumer contract on protected main; `tepp_api` HTTP interchange and loopback live listener implemented-main; production TLS remaining | partial |
