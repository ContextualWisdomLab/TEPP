...
| tenant/purpose/role/lifetime access and identity separation | ADR 0009; Threat Model | `tepp_api` time-bounded `PurposeGrant` and cross-tenant denial implemented-main; persistent `access_grant` storage and opaque-log integration remaining | partial |
