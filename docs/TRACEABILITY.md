...
| standalone + modular CWL MSA / no cross-service DB coupling | ADR 0011; `docs/API_CONTRACT.md` | current standalone crates; naruon HTTPS interchange on main; loopback live HTTP/1.1 listener on this PR; production TLS remaining | partial |
| naruon modular artifact consumer boundary | ADR 0011/0012; API contract | `docs/connectors/naruon-artifact-consumer.md` + PR #22 versioned consumer contract and HTTPS interchange on protected main; `tepp_api` `NaruonLiveService` loopback HTTP/1.1 listener on this PR; production TLS remaining | partial |
