...
| Simulation cutoff eligibility | `tepp_simulation` | pending-main (PR #62) | `available_time <= knowledge_cutoff` | delayed-document exclusion, generated-count agreement, exact-boundary admission, and fail-closed `TemporalInvariantViolation` for late documents | ADR 0002; `crates/tepp_simulation/tests/cutoff_eligibility_contract.rs`; `docs/research/simulation-cutoff-eligibility.md` |
