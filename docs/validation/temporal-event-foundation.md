...
| Bitemporal persistence + live SQL port | `persistence_postgres` | partial | event-instance SQL | migration contracts + recording transport + optional PgPool + live CI + tenant RLS + `0005` interval CHECKs + event-instance insert/as-known-at | Task 8 / PR #16 + #23 + #26 + #27 + #29 + #30–#35 + event-instance SQL |
| Bitemporal persistence + live SQL port | `persistence_postgres` | partial | source-artifact SQL + idempotent retry | migration contracts + recording transport + optional PgPool + live CI + tenant RLS isolation + source-artifact insert/lookup | Task 8 / PR #16 + #23 + #26 + #27 + #29 + tenant RLS + source-artifact SQL |
