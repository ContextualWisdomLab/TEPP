...
| Predicted-vs-observed contradiction | `prediction_contradiction` | active-PR | this PR | Allen `before`/`after` contradiction, `meets`/`met_by` unsupported, coverage required before unmatched predicted mass becomes fact, cutoff eligibility; label agreement is not RMSE recovery | ADR 0016 |
