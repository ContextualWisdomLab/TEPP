...
| Multiple-membership estimation rows | `membership_core` | implemented-main | rows + collapse refusal | 3-row RMSE + collapse deny | ADR 0003; `docs/research/membership-estimation-rows.md` |
...
| Adaptive orchestration router | `tepp_api` | implemented-main | mode selection, ablation, credential-free bind | mode selection, document-control denial, ablation, credential-free bind | ADR 0010; `docs/research/adaptive-orchestration-router.md` |
| Multilingual language-profile / concept gates | `concept_dictionary` | implemented-main | language-profile status + shared-concept RMSE | computed coordinate RMSE + translation/stopword/TF-IDF refusal | ADR 0004; `docs/research/multilingual-concept-alignment.md` |
