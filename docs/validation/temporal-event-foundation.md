...
| Recovery metrics | `validation_core` | implemented-main | — | RMSE/bias/coverage/MC gates | Task 11 / PR #19 |
 ...
 | TDT/CHRONOS evidence-status gates | `event_core` | active-PR | admission + first-story rates | known-stream miss/FA | ADR 0016; `docs/research/event-intelligence-status-gates.md` |
 | Purpose-bound provider payloads | `tepp_api` | accepted-target | active PR | expired/not-yet-valid/inverted/cross-tenant/impossible-calendar grant, mapping refusal, audited elevated re-id replay | ADR 0009; `docs/research/provider-payload-minimization.md` |
