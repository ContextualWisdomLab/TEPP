...
| Encrypted identity mapping envelope | `encrypted_mapping` | active-PR | this PR | recovered identity rate vs collapsed names + unauthorized-purpose, wrong-key-identity/bytes, tampered ciphertext/tag/nonce, key-redaction, empty-input, persistence-refusal, and nonce-reuse fail-closed probes; exact-head evidence with no unresolved security blocker before promotion | ADR 0009; `docs/research/encrypted-identity-mapping.md`; persistence waits for later migration |
