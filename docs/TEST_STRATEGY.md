...
- retain RED `f74d9ac11cb0acf3eb8fdd9ad79ac3d2e9180993` → repair `e9a7dee29afb97542bfe2965f850c8ab5a34368e`: once the canonical pair numerator is `u128`-admitted, `S1`, `S2`, and supported `n` are `u128` operands, so `n*S2` and `S1²` require no more than 256 bits. Product width alone cannot justify a post-Wide256 numerator refusal;
- retain represented-input reachability `5a19b6334487b43fb630abba7e487d7cf4c49960`: at `n=4096`, residual classes `{0,1,2^53}` have exact represented residual construction and exact distinct pairwise subtractions `{1,2^53,2^53-1}`, common dyadic shift zero, narrow-product overflow, exact 119-bit pair numerator `664_289_479_338_799_435_974_172_876_300_357_631`, and exact Wide256 recovery. The unreduced denominator `68_702_699_520` remains below `2^53`;
- retain exact-rounding-width characterization `a8423173188fa53a26a16d3afdafeb76e114cc1d`: at represented `n=2050`, the exact 118-bit pair numerator is recovered by Wide256 and denominator `8_610_922_500 <= 2^53`, but the existing exact candidate-square comparison needs 136-bit operands and the upward-adjacent midpoint comparison needs 140-bit operands. Test-only Wide256 comparisons prove seed `0x4296998e1aff78de` is below the target square while the target stays below the upward midpoint square. Widening only the numerator identity is therefore not end-to-end admission equivalence;
- before production widening, exercise the complete represented-input path with width-safe exact candidate-square and adjacent-midpoint comparisons, including exact tie cases and tie-to-even selection. Keep the pairwise path as a fail-closed reference/fallback until normalization, full-width cancellation, dyadic restoration, reduced denominator, candidate/midpoint comparison, and upstream exact-residual conversion are executable together and current-head GREEN;
