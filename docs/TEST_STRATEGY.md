...
- neutral zero is a translation origin, not synthetic evidence: the observed residual values are unchanged. Do not claim zero-plus-observed anchors are globally resource-optimal without a separate proof;
...
Until same-head correctness, exact-head gates, independent review, and resource measurements exist, production bias-SE exact admission remains `n=4..=16`.
