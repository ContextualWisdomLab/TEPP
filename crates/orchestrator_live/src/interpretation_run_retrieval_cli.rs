//! Operator loopback CLI for contextual-orchestrator interpretation-run GET-by-id.
//!
//! GAP-003A unique slice: operators run `tepp-interpretation-run-get get` to
//! mint `contextual_orchestrator_interpretation_run_retrieval_exchange` onto
//! spawned `tepp-orchestrator-loopback` TCP. Stdout is one metric-free
//! hypothetical identity with `claim_status=hypothetical` and
//! `scientific_authority=false`. `tepp.scientific_acceptance.v1` never
//! appears. The CLI does not infer causality or call a model provider. Naruon
//! and `LineageWeave` are refused. `NaruonLiveService` stays POST-only. This
//! module does not duplicate interpretation-run CLI (#425), collection GET
//! (#433), collection CLI (#436), GET-by-id HTTP (#438), project-history
//! retrieval CLI (#431), analysis-run GET-by-id (#359), Leiden, or GAP-010
...
use crate::interpretation_run_retrieval_http::{
    ...
    InterpretationRunRetrievalHttpExchange,
};
use crate::request::{
...
pub enum InterpretationRunRetrievalCliVerb {
    /// `GET /v1/interpretation-runs/{idempotency_key}`.
    Get,
}

impl InterpretationRunRetrievalCliVerb {
...
pub struct InterpretationRunRetrievalCliInvocation {
    /// CLI verb to execute.
    pub verb: InterpretationRunRetrievalCliVerb,
    /// Loopback `host:port` of `tepp-orchestrator-loopback`.
    pub host: String,
    /// Published HTTPS origin used to mint the typed retrieval exchange.
    pub origin: String,
    /// Published modular consumer. Retrieval GET admits
...
impl InterpretationRunRetrievalCliInvocation {
        ...
        S: AsRef<str>,
    {
        ...
        let verb = InterpretationRunRetrievalCliVerb::parse(verb_token)?;
...
fn assemble_invocation(
    verb: InterpretationRunRetrievalCliVerb,
    flags: ParsedFlags,
    body: String,
) -> Result<InterpretationRunRetrievalCliInvocation, OrchestratorLiveError> {
    let invocation = InterpretationRunRetrievalCliInvocation {
        verb,
        host: flags
            .host
            .ok_or(OrchestratorLiveError::InvalidWirePayload)?,
...
/// GET `/v1/interpretation-runs/{idempotency_key}` with an empty body.
pub fn loopback_http1_from_interpretation_run_retrieval_exchange(
    exchange: &InterpretationRunRetrievalHttpExchange,
    loopback_host: &str,
    ...
    let path = rest
        ...
        .ok_or(OrchestratorLiveError::InvalidWirePayload)?;
    let _idempotency_key = interpretation_run_retrieval_path_id(path)?;
...
/// [`InterpretationRunRetrievalCliInvocation::validate`].
pub fn compose_interpretation_run_retrieval_cli_http(
    ...
    loopback_http1_from_interpretation_run_retrieval_exchange(&exchange, &invocation.host)
}
...
pub fn dispatch_interpretation_run_retrieval_cli(
    service: &mut OrchestratorLiveService,
    invocation: &InterpretationRunRetrievalCliInvocation,
) -> Result<OrchestratorLiveResponse, OrchestratorLiveError> {
    let request = compose_interpretation_run_retrieval_cli_http(invocation)?;
...
pub fn execute_interpretation_run_retrieval_cli(
    invocation: &InterpretationRunRetrievalCliInvocation,
) -> Result<OrchestratorLiveResponse, OrchestratorLiveError> {
...
pub fn render_interpretation_run_retrieval_cli_stdout(
    invocation: &InterpretationRunRetrievalCliInvocation,
...
pub fn read_interpretation_run_retrieval_cli_stdin(
...
mod tests {
    use super::{
        compose_interpretation_run_retrieval_cli_http,
        loopback_http1_from_interpretation_run_retrieval_exchange,
        read_interpretation_run_retrieval_cli_stdin, InterpretationRunRetrievalCliInvocation,
        InterpretationRunRetrievalCliVerb,
    };
    use crate::interpretation_run_cli::CONTEXTUAL_ORCHESTRATOR_CONSUMER_CODE;
    use crate::{
        contextual_orchestrator_interpretation_run_retrieval_exchange, OrchestratorLiveError,
    };
    ...
    fn from_args_mints_get_and_refuses_fail_closed_inputs() {
        assert_eq!(
            InterpretationRunRetrievalCliVerb::parse("get").expect("get"),
            InterpretationRunRetrievalCliVerb::Get
        );
        assert_eq!(InterpretationRunRetrievalCliVerb::Get.as_str(), "get");
        assert_eq!(
            InterpretationRunRetrievalCliVerb::parse("list"),
            Err(OrchestratorLiveError::InvalidWirePayload)
        );
        let get = InterpretationRunRetrievalCliInvocation::from_args(get_args(), "").expect("get");
        assert_eq!(get.verb, InterpretationRunRetrievalCliVerb::Get);
        let http = compose_interpretation_run_retrieval_cli_http(&get).expect("http");
        assert!(http.starts_with("GET /v1/interpretation-runs/idem-a HTTP/1.1"));
    ...
    fn from_args_refuses_unpublished_body_pagination_and_non_get() {
        ...
        assert_eq!(
            InterpretationRunRetrievalCliInvocation::from_args(get_args(), "{}").unwrap_err(),
            OrchestratorLiveError::InvalidWirePayload
        );
        assert_eq!(
            InterpretationRunRetrievalCliInvocation::from_args(
        ...
        let leftover =
            read_interpretation_run_retrieval_cli_stdin(false, std::io::Cursor::new(b"leftover"))
                .expect("leftover");
        assert_eq!(leftover, "leftover");
        assert!(
            read_interpretation_run_retrieval_cli_stdin(true, std::io::empty())
                .expect("tty")
                .is_empty()
        );
        let exchange =
            contextual_orchestrator_interpretation_run_retrieval_exchange(ORIGIN, "idem-a")
                .expect("exchange");
        let mut posted = exchange.clone();
        posted.method = "POST";
        assert_eq!(
            loopback_http1_from_interpretation_run_retrieval_exchange(&posted, "127.0.0.1:18082")
                .unwrap_err(),
            OrchestratorLiveError::InvalidWirePayload
        );
    }
