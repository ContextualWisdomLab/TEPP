...
//! `tepp-interpretation-runs` CLI mints typed contextual-orchestrator POST and
//! collection GET exchanges onto spawned `tepp-orchestrator-loopback` TCP.

mod error;
mod http;
mod interpretation_run_cli;
mod interpretation_run_collection_cli;
...
/// Compose HTTP/1.1 interpretation-run collection GET from a CLI invocation.
pub use interpretation_run_collection_cli::compose_interpretation_run_collection_cli_http;
/// Dispatch an interpretation-run collection CLI invocation in-process.
pub use interpretation_run_collection_cli::dispatch_interpretation_run_collection_cli;
/// Execute an interpretation-run collection CLI invocation over loopback TCP.
pub use interpretation_run_collection_cli::execute_interpretation_run_collection_cli;
/// Render a typed collection GET exchange onto a loopback HTTP/1.1 request.
pub use interpretation_run_collection_cli::loopback_http1_from_interpretation_run_collection_exchange;
/// Read leftover stdin for the interpretation-run collection CLI.
pub use interpretation_run_collection_cli::read_interpretation_run_collection_cli_stdin;
/// Filter collection CLI stdout so listed rows stay hypothetical identities.
pub use interpretation_run_collection_cli::render_interpretation_run_collection_cli_stdout;
/// Loopback interpretation-run collection CLI invocation.
pub use interpretation_run_collection_cli::InterpretationRunCollectionCliInvocation;
/// Loopback interpretation-run collection CLI verb.
pub use interpretation_run_collection_cli::InterpretationRunCollectionCliVerb;
