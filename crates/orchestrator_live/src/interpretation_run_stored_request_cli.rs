//! Operator loopback CLI for contextual-orchestrator stored-request GET.
//!
//! GAP-003A unique slice: operators run `tepp-interpretation-run-request get`
//! to mint `contextual_orchestrator_interpretation_run_stored_request_exchange`
...
//! This module does not duplicate stored-request GET (#453), GET-by-id (#438),
//! retrieval CLI (#439), collection GET/CLI (#433/#436), create CLI (#425),
//! analysis-run stored-request CLI (#395), cancel lineages (closed), Leiden,
...
pub enum InterpretationRunStoredRequestCliVerb {
    /// `GET /v1/interpretation-runs/{idempotency_key}/request`.
    Get,
}

impl InterpretationRunStoredRequestCliVerb {
...
pub struct InterpretationRunStoredRequestCliInvocation {
    /// CLI verb to execute.
    pub verb: InterpretationRunStoredRequestCliVerb,
...
impl InterpretationRunStoredRequestCliInvocation {
        ...
        S: AsRef<str>,
    {
        ...
        let verb = InterpretationRunStoredRequestCliVerb::parse(verb_token)?;
...
fn assemble_invocation(
    ...
    flags: ParsedFlags,
    body: String,
) -> Result<InterpretationRunStoredRequestCliInvocation, OrchestratorLiveError> {
    let invocation = InterpretationRunStoredRequestCliInvocation {
        verb,
        host: flags
            .host
            .ok_or(OrchestratorLiveError::InvalidWirePayload)?,
...
/// GET `/v1/interpretation-runs/{idempotency_key}/request` with an empty body.
pub fn loopback_http1_from_interpretation_run_stored_request_exchange(
    exchange: &InterpretationRunStoredRequestHttpExchange,
    loopback_host: &str,
    ...
    let path = rest
        ...
        .ok_or(OrchestratorLiveError::InvalidWirePayload)?;
    let _idempotency_key = interpretation_run_stored_request_path_id(path)?;
...
/// Compose one HTTP/1.1 stored-request GET from the typed consumer exchange.
///
/// # Errors
///
/// Returns the same fail-closed errors as
/// [`InterpretationRunStoredRequestCliInvocation::validate`].
pub fn compose_interpretation_run_stored_request_cli_http(
    invocation: &InterpretationRunStoredRequestCliInvocation,
) -> Result<String, OrchestratorLiveError> {
    invocation.validate()?;
    ...
    loopback_http1_from_interpretation_run_stored_request_exchange(&exchange, &invocation.host)
}
...
pub fn dispatch_interpretation_run_stored_request_cli(
    service: &mut OrchestratorLiveService,
    invocation: &InterpretationRunStoredRequestCliInvocation,
) -> Result<OrchestratorLiveResponse, OrchestratorLiveError> {
    let request = compose_interpretation_run_stored_request_cli_http(invocation)?;
...
pub fn execute_interpretation_run_stored_request_cli(
    invocation: &InterpretationRunStoredRequestCliInvocation,
) -> Result<OrchestratorLiveResponse, OrchestratorLiveError> {
...
pub fn render_interpretation_run_stored_request_cli_stdout(
    invocation: &InterpretationRunStoredRequestCliInvocation,
    ...
    if stored.idempotency_key() != invocation.idempotency_key {
        return Err(OrchestratorLiveError::InvalidWirePayload);
    }
    stored.to_json()
}
...
pub fn read_interpretation_run_stored_request_cli_stdin(
...
mod tests {
    ...
    fn from_args_mints_get_and_refuses_fail_closed_inputs() {
        assert_eq!(
            InterpretationRunStoredRequestCliVerb::parse("get").expect("get"),
            InterpretationRunStoredRequestCliVerb::Get
        );
        assert_eq!(InterpretationRunStoredRequestCliVerb::Get.as_str(), "get");
        assert_eq!(
    ...
    fn from_args_refuses_unpublished_body_pagination_and_non_get() {
        ...
        assert_eq!(
            InterpretationRunStoredRequestCliInvocation::from_args(get_args(), "{}").unwrap_err(),
            OrchestratorLiveError::InvalidWirePayload
        );
        assert_eq!(
            InterpretationRunStoredRequestCliInvocation::from_args(
        ...
        let exchange =
            contextual_orchestrator_interpretation_run_stored_request_exchange(ORIGIN, "idem-a")
                .expect("exchange");
