//! Contract tests for contextual-orchestrator interpretation-run stored-request GET.
...
fn stored_request_exchange_is_metric_free_get_without_credentials() {
    ...
    assert_eq!(
        interpretation_run_stored_request_path_id(
            "/v1/interpretation-runs/orch-live-idem-001/request"
        )
        .expect("id"),
        "orch-live-idem-001"
    );
...
fn live_get_returns_stored_request_without_scientific_authority() {
    ...
    let got = service.handle_http_request(
        "GET /v1/interpretation-runs/orch-live-idem-001/request HTTP/1.1\r\nHost: 127.0.0.1\r\ncontent-type: application/json\r\ntepp-consumer: contextual-orchestrator\r\ntepp-contract-version: 1\r\ncontent-length: 0\r\n\r\n",
    );
...
#[test]
fn stored_request_serves_over_tcp() {
    let request = sample_request();
    ...
    let http = "GET /v1/interpretation-runs/orch-live-idem-001/request HTTP/1.1\r\nHost: 127.0.0.1\r\ncontent-type: application/json\r\ntepp-consumer: contextual-orchestrator\r\ntepp-contract-version: 1\r\ncontent-length: 0\r\n\r\n";
