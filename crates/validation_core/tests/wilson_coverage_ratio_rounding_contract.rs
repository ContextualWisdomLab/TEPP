...
fn durable_coverage_ratio_is_correctly_rounded_before_wilson_projection() {
    ...
    // The same exact denominator provenance also matters inside the Wilson
    // projection: evaluating through exact 1/n moves the lower endpoint two ULPs
    // below the predecessor fixture that first repaired only the coverage ratio.
    let json = r#"{
        ...
        "wilson_lower":0.49999998967401094,
        "wilson_upper":0.5000000103259667
    }"#;

    let evidence: WilsonCoverageEvidenceV1 = serde_json::from_str(json)
        .expect("exact count provenance must determine coverage and Wilson projection");

    assert_eq!(evidence.empirical_coverage.to_bits(), 0x3fdf_ffff_ffff_ff37);
    assert_eq!(evidence.wilson_lower.to_bits(), 0x3fdf_ffff_f4e9_9d1e);
