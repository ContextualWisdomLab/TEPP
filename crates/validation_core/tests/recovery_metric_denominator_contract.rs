...
fn recovery_metric_summary_preserves_attempted_failure_denominator() {
    ...
    assert_eq!(summary.successful_metric_mean(), Some(successful.mean));
}
...
fn recovery_metric_summary_retains_catastrophic_failure_without_fabricating_metric() {
    ...
    assert_eq!(summary.successful_metric_mean(), None);
    assert_eq!(summary.successful_metric_summary(), None);
}

#[test]
fn recovery_metric_summary_retains_one_success_without_inventing_mc_uncertainty() {
    let summary = summarize_recovery_metric_replications(4, &[0.25], 0.025, 0.975)
        .expect("one-success recovery summary");

    assert_eq!(summary.attempted_replication_count(), 4);
    assert_eq!(summary.successful_replication_count(), 1);
    assert_eq!(summary.failure_count(), 3);
    assert!((summary.failure_rate() - 0.75).abs() < 1.0e-12);
    assert_eq!(summary.successful_metric_mean(), Some(0.25));
    assert_eq!(
        summary.successful_metric_summary(),
        None,
        "one successful replication cannot estimate between-replication SD or Monte Carlo SE"
    );
}
