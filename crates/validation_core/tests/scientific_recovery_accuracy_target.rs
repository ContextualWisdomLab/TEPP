...
use validation_core::{ValidationError, promote_scientific_recovery, rmse_standard_error, root_mean_square_error};
...
fn practical_rmse_inside_target_is_not_rejected_for_being_precisely_nonzero() {
    let truth = [0.0; 8];
    let recovered = [0.02, -0.02, 0.02, -0.02, 0.02, -0.02, 0.02, -0.02];
...
fn monte_carlo_uncertainty_remains_binding_near_practical_rmse_target() {
    let truth = [0.0; 8];
    let recovered = [0.0, 0.06, 0.0, 0.06, 0.0, 0.06, 0.0, 0.06];
    let rmse = root_mean_square_error(&truth, &recovered).expect("rmse");
    let rmse_se = rmse_standard_error(&truth, &recovered).expect("rmse se");
