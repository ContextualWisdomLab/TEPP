...
/// coordinates belong inside one replication and whether the outer replications
/// are scientifically independent. This boundary deliberately does not infer an
/// effective sample size from row, cluster, membership, or time counts.
///
/// # Errors
///
/// Returns [`ValidationError::InvalidInput`] for an empty replication set,
/// mismatched replication counts, or an invalid inner truth/recovery pair. The
/// underlying exact-head claim gate also rejects a non-zero recovery when fewer
/// than two independent replications leave Monte Carlo uncertainty undefined,
/// invalid accuracy/uncertainty configuration, head mismatch, or a conservative
...
pub fn promote_scientific_recovery(
    ...
    if truth_replications.is_empty() || truth_replications.len() != recovered_replications.len() {
