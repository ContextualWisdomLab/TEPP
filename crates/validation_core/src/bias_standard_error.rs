...
fn compare_scaled_ratio_to_dyadic_square(
    ...
    if exponent_delta >= 0 {
        let left = multiply_by_power_of_two(numerator, exponent_delta as u32)?;
        Some(left.cmp(&right))
    } else {
        let shifted_right = multiply_by_power_of_two(right, (-exponent_delta) as u32)?;
...
fn midpoint_dyadic(left: f64, right: f64) -> Option<(u128, i32)> {
    ...
    let left_shift = left_exponent.checked_sub(common_exponent)? as u32;
    let right_shift = right_exponent.checked_sub(common_exponent)? as u32;
...
fn exact_power_of_two(exponent: i32) -> Option<f64> {
    if (-1022..=1023).contains(&exponent) {
        return Some(f64::from_bits(((exponent + 1023) as u64) << 52));
    }
    if (-1074..=-1023).contains(&exponent) {
        return Some(f64::from_bits(1_u64 << (exponent + 1074)));
    }
    None
}
...
fn correctly_rounded_scaled_sqrt_ratio(
    ...
    if numerator == 0 || denominator == 0 || numerator > (1_u128 << 53) {
        return None;
    }
    let unit = exact_power_of_two(unit_exponent)?;
    let denominator_f64 = denominator as f64;
    if denominator_f64 as u128 != denominator {
        return None;
    }
...
fn exact_four_observation_standard_error(
    ...
    for dyadic in pair_dyadics.into_iter().flatten() {
        let shift = dyadic.1.checked_sub(unit_exponent)? as u32;
