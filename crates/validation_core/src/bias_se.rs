...
fn exact_pair_distance_standard_error(
    ...
    // cheaper exact identities in `bias.rs`; four through fifteen observations are
    // the smallest remaining sample sizes with demonstrated one-ULP errors in
    // the translated floating moment/sqrt path.
    if truth.len() != recovered.len() || !(4..=15).contains(&truth.len()) {
        return None;
    }
...
///
/// Four- through fifteen-observation samples whose represented residuals and
