//! Operator loopback CLI for `LineageWeave` project-history stored-request GET.
//!
//! GAP-003A unique slice: operators run `tepp-project-history-request get` to mint
//! `lineageweave_project_history_stored_request_exchange` onto spawned
//! `tepp-loopback` TCP. Stdout is the stored `ProjectHistoryRequest`.
//! Stored projection `inference_status` remains `temporal_association_only`.
...
//! `NaruonLiveService` stays POST-only. This module does not duplicate stored-request GET (#455), GET-by-id
//! HTTP (#429), retrieval CLI (#431), collection GET/CLI (#424/#428),
//! POST CLI (#420), interpretation-run stored-request CLI (#454), cancel
...
use crate::{
    ...
    PROJECT_HISTORY_RETRIEVAL_TENANT_HEADER, ProjectHistoryRequest,
    lineageweave_project_history_stored_request_exchange, project_history_stored_request_path_id,
    refuse_metrics_on_project_history_stored_request_payload,
};

const MAXIMUM_HTTP_RESPONSE_BYTES: usize =
    NARUON_LIVE_HEADER_BYTE_LIMIT + 4 + DEFAULT_PROJECT_HISTORY_BYTE_LIMIT;

/// Supported operator verbs for the loopback project-history stored-request CLI.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ProjectHistoryStoredRequestCliVerb {
    /// `GET /v1/project-histories/{idempotency_key}/request`.
    Get,
}

impl ProjectHistoryStoredRequestCliVerb {
...
/// One operator CLI invocation against a loopback project-history stored-request listener.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ProjectHistoryStoredRequestCliInvocation {
    /// CLI verb to execute.
    pub verb: ProjectHistoryStoredRequestCliVerb,
    ...
    /// Tenant workspace that owns the stored create request.
    pub tenant_workspace_id: String,
    /// JSON body. Stored-request GET requires empty.
    pub body: String,
}

impl ProjectHistoryStoredRequestCliInvocation {
        ...
        S: AsRef<str>,
    {
        ...
        let verb = ProjectHistoryStoredRequestCliVerb::parse(verb_token)?;
    ...
    /// empty, unpublished, naruon, nonempty-body, or oversized fields.
    pub fn validate(&self) -> Result<(), ApiError> {
        ...
        refuse_metrics_on_project_history_stored_request_payload(&self.body)?;
        Ok(())
    }
}
...
fn assemble_invocation(
    verb: ProjectHistoryStoredRequestCliVerb,
    flags: ParsedFlags,
    body: String,
) -> Result<ProjectHistoryStoredRequestCliInvocation, ApiError> {
    let invocation = ProjectHistoryStoredRequestCliInvocation {
        verb,
...
/// the loopback bind address. Public bind hosts fail closed. GET-by-id, collection, cancel extra-segments, and pagination headers fail closed.
///
/// # Errors
///
...
/// exchange is not a GET `/v1/project-histories/{idempotency_key}/request` with an
/// empty body.
pub fn loopback_http1_from_project_history_stored_request_exchange(
    ...
    let path = rest
        ...
        .ok_or(ApiError::InvalidWirePayload)?;
    let _idempotency_key = project_history_stored_request_path_id(path)?;
...
/// [`ProjectHistoryStoredRequestCliInvocation::validate`].
pub fn compose_project_history_stored_request_cli_http(
    invocation: &ProjectHistoryStoredRequestCliInvocation,
) -> Result<String, ApiError> {
    invocation.validate()?;
    ...
    loopback_http1_from_project_history_stored_request_exchange(&exchange, &invocation.host)
}
...
pub fn dispatch_project_history_stored_request_cli(
    service: &mut AnalysisRunLiveService,
    invocation: &ProjectHistoryStoredRequestCliInvocation,
) -> Result<NaruonLiveResponse, ApiError> {
    let request = compose_project_history_stored_request_cli_http(invocation)?;
...
pub fn execute_project_history_stored_request_cli(
    invocation: &ProjectHistoryStoredRequestCliInvocation,
) -> Result<NaruonLiveResponse, ApiError> {
...
///
/// Evidence text belongs to the stored create request and is admitted.
/// RMSE, bias, coverage, SE-gate, and causal-score keys fail closed.
///
/// # Errors
///
/// Returns [`ApiError::InvalidWirePayload`] when a body carries metric keys,
/// `tepp.scientific_acceptance.v1`, a mismatched identity, or a
/// success body that is not a stored `ProjectHistoryRequest`.
pub fn render_project_history_stored_request_cli_stdout(
    invocation: &ProjectHistoryStoredRequestCliInvocation,
    ...
    refuse_metrics_on_project_history_stored_request_payload(&response.body)?;
    ...
    let stored = ProjectHistoryRequest::from_json(&response.body)?;
    if stored.tenant_workspace_id != invocation.tenant_workspace_id
        || stored.idempotency_key != invocation.idempotency_key
    {
        return Err(ApiError::InvalidWirePayload);
    }
    stored.to_json()
...
/// wire limit.
pub fn read_project_history_stored_request_cli_stdin(
...
mod branch_coverage_tests {
    ...
    use super::{
        ProjectHistoryStoredRequestCliInvocation, ProjectHistoryStoredRequestCliVerb,
        loopback_http1_from_project_history_stored_request_exchange, parse_http_response,
        read_project_history_stored_request_cli_stdin, valid_http_field_name,
    };
    use crate::{
    ...
    fn invocation() -> ProjectHistoryStoredRequestCliInvocation {
        ProjectHistoryStoredRequestCliInvocation {
            verb: ProjectHistoryStoredRequestCliVerb::Get,
    ...
    fn invocation_and_flag_error_arms_are_covered() {
        ...
        assert!(super::compose_project_history_stored_request_cli_http(&value).is_err());
        ...
        for args in [
            ...
            assert!(ProjectHistoryStoredRequestCliInvocation::from_args(args, "").is_err());
        }
    }
    ...
    fn exchange_header_and_target_error_arms_are_covered() {
        ...
        let base =
            lineageweave_project_history_stored_request_exchange(origin, "tenant-a", "idem-a")
                .expect("exchange");
    ...
    fn response_parser_and_reader_error_arms_are_covered() {
        ...
        assert!(read_project_history_stored_request_cli_stdin(false, Cursor::new([0xff])).is_err());
        assert!(
        ...
        assert!(read_project_history_stored_request_cli_stdin(false, FailingReader).is_err());
