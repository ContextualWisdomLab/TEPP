//! Operator loopback CLI for analysis-run retry POST.
//!
//! GAP-003A retry CLI slice: operators run `tepp-retry retry` to clone a
//! failed or cancelled run from the typed naruon/`LineageWeave` retry exchange
//! onto spawned `tepp-loopback` TCP. Stdout is a metric-free child `202
//! Accepted`. `tepp.scientific_acceptance.v1` never appears. Persistence
//! remains GAP-003B.
...
use crate::{
    lineageweave_analysis_run_retry_exchange, naruon_analysis_run_retry_exchange,
    refuse_metrics_on_retry_payload, AnalysisRunAccepted, AnalysisRunLiveService,
    AnalysisRunRetryRequest, ApiError, NaruonHttpExchange, NaruonLiveResponse,
    ANALYSIS_RUN_RETRY_ID_MAX_LEN, LINEAGEWEAVE_CONSUMER_CODE, NARUON_CONSUMER_CODE,
    NARUON_LIVE_IO_TIMEOUT,
};

const SCIENTIFIC_ACCEPTANCE_SCHEMA: &str = "tepp.scientific_acceptance.v1";

/// Supported operator verbs for the loopback retry CLI.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum AnalysisRunRetryCliVerb {
    /// `POST /v1/analysis-runs/{run_id}/retry`.
    Retry,
}

impl AnalysisRunRetryCliVerb {
...
/// One operator CLI invocation against a loopback retry POST listener.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct AnalysisRunRetryCliInvocation {
    /// CLI verb to execute.
    pub verb: AnalysisRunRetryCliVerb,
    /// Loopback `host:port` of `tepp-loopback`.
    pub host: String,
    /// Published HTTPS origin used to mint the typed retry exchange.
    pub origin: String,
    ...
    /// Optional typed retry JSON. Empty POST is admitted.
    pub body: String,
}

impl AnalysisRunRetryCliInvocation {
    /// Parse argv plus stdin body into a validated loopback retry invocation.
    ///
    /// # Errors
    ///
    ...
    /// credential-shaped flags, hostile identities, metric bodies, or a typed
    /// body that does not match the path identity and new idempotency key.
        ...
        S: AsRef<str>,
    {
        ...
        let verb = AnalysisRunRetryCliVerb::parse(verb_token)?;
        let flags = parse_flags(rest)?;
        assemble_invocation(verb, flags, body.into())
    }

    /// Reject a non-loopback host, unpublished consumer, or hostile retry body.
    ///
    /// # Errors
    ///
    ...
    pub fn validate(&self) -> Result<(), ApiError> {
        ...
        refuse_metrics_on_retry_payload(&self.body)?;
        if !self.body.is_empty() {
            let request = AnalysisRunRetryRequest::from_json(&self.body)?;
...
fn assemble_invocation(
    verb: AnalysisRunRetryCliVerb,
    flags: ParsedFlags,
    body: String,
) -> Result<AnalysisRunRetryCliInvocation, ApiError> {
...
fn retry_request(
    invocation: &AnalysisRunRetryCliInvocation,
) -> Result<AnalysisRunRetryRequest, ApiError> {
    if invocation.body.is_empty() {
        AnalysisRunRetryRequest::new(&invocation.run_id, &invocation.idempotency_key)
    } else {
        AnalysisRunRetryRequest::from_json(&invocation.body)
    }
}
...
fn retry_exchange(
    ...
    let request = retry_request(invocation)?;
    if invocation.consumer == LINEAGEWEAVE_CONSUMER_CODE {
        lineageweave_analysis_run_retry_exchange(&invocation.origin, &request)
    } else if invocation.consumer == NARUON_CONSUMER_CODE {
        naruon_analysis_run_retry_exchange(&invocation.origin, &request)
    } else {
        Err(ApiError::InvalidWirePayload)
    }
}

/// Render a typed retry exchange as HTTP/1.1 for a bound loopback listener.
///
...
/// exchange is not a POST `/retry`.
pub fn loopback_http1_from_retry_exchange(
    ...
    let path = rest
        ...
        .ok_or(ApiError::InvalidWirePayload)?;
    if path.rsplit('/').next() != Some("retry") {
        return Err(ApiError::InvalidWirePayload);
    }
...
/// Compose one HTTP/1.1 retry POST from the typed consumer exchange.
///
/// # Errors
///
/// Returns the same fail-closed errors as
/// [`AnalysisRunRetryCliInvocation::validate`].
pub fn compose_analysis_run_retry_cli_http(
    invocation: &AnalysisRunRetryCliInvocation,
) -> Result<String, ApiError> {
    invocation.validate()?;
    let exchange = retry_exchange(invocation)?;
    loopback_http1_from_retry_exchange(&exchange, &invocation.host)
}

/// Dispatch one retry CLI invocation against an in-process loopback service.
///
/// # Errors
///
/// Returns fail-closed validation errors before the HTTP handler runs.
pub fn dispatch_analysis_run_retry_cli(
    service: &mut AnalysisRunLiveService,
    invocation: &AnalysisRunRetryCliInvocation,
) -> Result<NaruonLiveResponse, ApiError> {
    let request = compose_analysis_run_retry_cli_http(invocation)?;
    Ok(service.handle_http_request(&request))
}

/// Execute one retry CLI invocation over loopback TCP against `tepp-loopback`.
///
/// # Errors
///
/// Returns fail-closed validation, transport, or response-framing errors.
pub fn execute_analysis_run_retry_cli(
    invocation: &AnalysisRunRetryCliInvocation,
) -> Result<NaruonLiveResponse, ApiError> {
...
/// Filter CLI stdout so retry receipts never print scientific acceptance.
///
/// # Errors
///
/// Returns [`ApiError::InvalidWirePayload`] when a receipt carries metric keys,
/// `tepp.scientific_acceptance.v1`, or a success body that is not a new child
/// `202 Accepted`.
pub fn render_analysis_run_retry_cli_stdout(
    invocation: &AnalysisRunRetryCliInvocation,
    ...
    refuse_metrics_on_retry_payload(&response.body)?;
    ...
    if accepted.run_id == invocation.run_id
        || accepted.idempotency_key != invocation.idempotency_key
        || accepted.run_state != "accepted"
    {
        return Err(ApiError::InvalidWirePayload);
    }
    accepted.to_json()
...
/// Read stdin leftover bytes on a non-terminal; empty retry POST is admitted.
///
/// # Errors
///
/// Returns [`ApiError::InvalidWirePayload`] when stdin cannot be read.
pub fn read_analysis_run_retry_cli_stdin(
