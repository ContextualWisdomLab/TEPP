...
/// review, Copilot, NIM/NVIDIA, proxy, or bearer credential. Other failures
/// match [`naruon_analysis_run_exchange`].
...
mod tests {
    ...
    fn refuse_credential_headers_covers_reserved_and_secret_names() {
        ...
        assert_eq!(
            refuse_credential_headers(&[("x-nim-key", "k")]),
            Err(ApiError::AuthorizationDenied)
        );
        assert_eq!(
            refuse_credential_headers(&[("x-nvidia-session", "s")]),
            Err(ApiError::AuthorizationDenied)
        );
    }
