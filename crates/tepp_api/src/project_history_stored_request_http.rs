//! Provider-owned project-history stored-request GET contracts.
//!
//! GAP-003A unique slice: `GET /v1/project-histories/{idempotency_key}/request`
...
//! This module does not duplicate GET-by-id (#429), retrieval CLI (#431),
//! collection GET/CLI (#424/#428), POST CLI (#420), interpretation-run
//! stored-request GET (#453), cancel lineages (closed), Leiden, or GAP-010
//! Figma/export. Persistence remains GAP-003B. Naruon is refused.
...
use crate::project_history_retrieval_http::{
    PROJECT_HISTORY_RETRIEVAL_ID_MAX_LEN, PROJECT_HISTORY_RETRIEVAL_TENANT_HEADER,
};
...
/// `GET /v1/project-histories/{key}/request`.
///
/// # Errors
///
...
pub fn project_history_stored_request_path_id(path: &str) -> Result<String, ApiError> {
    let remainder = path
...
#[must_use]
pub fn is_project_history_stored_request_path(path: &str) -> bool {
    project_history_stored_request_path_id(path).is_ok()
}
...
/// key is present.
pub fn refuse_metrics_on_project_history_stored_request_payload(
    payload: &str,
) -> Result<(), ApiError> {
...
/// # Errors
///
/// Returns a fail-closed origin, tenant, or identity error.
pub fn lineageweave_project_history_stored_request_exchange(
    origin: &str,
    ...
    let target_path = format!("{PROJECT_HISTORY_PATH}/{encoded_id}/request");
...
mod tests {
    use super::{
        is_project_history_stored_request_path,
        lineageweave_project_history_stored_request_exchange,
        project_history_stored_request_path_id,
    };
    use crate::ApiError;
    ...
    fn stored_request_exchange_is_lineageweave_get_without_credentials() {
        ...
        assert!(is_project_history_stored_request_path(
            "/v1/project-histories/idem-a/request"
        ));
        assert!(!is_project_history_stored_request_path(
            "/v1/project-histories/idem-a"
        ));
        assert_eq!(
            project_history_stored_request_path_id("/v1/project-histories/idem-a/request")
                .expect("id"),
            "idem-a"
        );
        assert_eq!(
