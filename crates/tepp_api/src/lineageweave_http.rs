...
use crate::analysis_run_retry_parent_http::naruon_analysis_run_retry_parent_exchange;
...
/// Build a `LineageWeave` → TEPP retry-parent GET without credentials.
///
...
/// a metric-free inspect of the parent identity, not a measurement result.
///
/// # Errors
///
/// Returns the same fail-closed errors as [`naruon_analysis_run_retry_parent_exchange`].
pub fn lineageweave_analysis_run_retry_parent_exchange(
    origin: &str,
    run_id: &str,
) -> Result<NaruonHttpExchange, ApiError> {
    let mut exchange = naruon_analysis_run_retry_parent_exchange(origin, run_id)?;
    let consumer_header = exchange
        .headers
        .iter_mut()
        .find(|(name, _)| name.eq_ignore_ascii_case("tepp-consumer"))
...
mod tests {
    use super::{
        ...
        lineageweave_analysis_run_exchange, lineageweave_analysis_run_retry_parent_exchange,
    };
    ...
    #[test]
    fn lineageweave_retry_parent_exchange_swaps_only_the_consumer_header() {
        ...
        assert_eq!(
            exchange.target_url,
            "https://tepp.example.test/v1/analysis-runs/tepp-run-1/parent"
        );
        ...
        assert_eq!(
            lineageweave_analysis_run_retry_parent_exchange("https://tepp.example.test", ""),
            Err(ApiError::InvalidWirePayload)
        );
    }
}
