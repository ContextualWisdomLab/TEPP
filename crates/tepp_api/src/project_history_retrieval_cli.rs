//! Operator loopback CLI for `LineageWeave` project-history GET-by-id.
//!
//! GAP-003A unique slice: operators run `tepp-project-history-get get` to mint
//! `lineageweave_project_history_retrieval_exchange` onto spawned
//! `tepp-loopback` TCP. Stdout is one stored cutoff-safe
//! `ProjectHistoryProjection` with `inference_status=temporal_association_only`.
...
//! project-history POST CLI (#420), temporal-context CLI (#414), export
//! retrieval CLI (#417), stored-request CLI (#395), analysis-run GET-by-id,
//! Leiden, or GAP-010 Figma/export. Persistence remains GAP-003B.
...
/// Supported operator verbs for the loopback project-history retrieval CLI.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ProjectHistoryRetrievalCliVerb {
    /// `GET /v1/project-histories/{idempotency_key}`.
    Get,
}

impl ProjectHistoryRetrievalCliVerb {
...
/// One operator CLI invocation against a loopback project-history GET-by-id listener.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ProjectHistoryRetrievalCliInvocation {
    /// CLI verb to execute.
    pub verb: ProjectHistoryRetrievalCliVerb,
...
impl ProjectHistoryRetrievalCliInvocation {
        ...
        S: AsRef<str>,
    {
        ...
        let verb = ProjectHistoryRetrievalCliVerb::parse(verb_token)?;
...
fn assemble_invocation(
    ...
    flags: ParsedFlags,
    body: String,
) -> Result<ProjectHistoryRetrievalCliInvocation, ApiError> {
    let invocation = ProjectHistoryRetrievalCliInvocation {
        verb,
...
/// the loopback bind address. Public bind hosts fail closed. Collection paths
/// and pagination headers fail closed.
///
/// # Errors
///
...
/// exchange is not a GET `/v1/project-histories/{idempotency_key}` with an
/// empty body.
pub fn loopback_http1_from_project_history_retrieval_exchange(
    ...
    let path = rest
        ...
        .ok_or(ApiError::InvalidWirePayload)?;
    let _idempotency_key = project_history_retrieval_path_id(path)?;
...
/// [`ProjectHistoryRetrievalCliInvocation::validate`].
pub fn compose_project_history_retrieval_cli_http(
    invocation: &ProjectHistoryRetrievalCliInvocation,
) -> Result<String, ApiError> {
    invocation.validate()?;
...
pub fn dispatch_project_history_retrieval_cli(
    service: &mut AnalysisRunLiveService,
    invocation: &ProjectHistoryRetrievalCliInvocation,
) -> Result<NaruonLiveResponse, ApiError> {
    let request = compose_project_history_retrieval_cli_http(invocation)?;
    Ok(service.handle_http_request(&request))
}

/// Execute one retrieval CLI invocation over loopback TCP against `tepp-loopback`.
///
/// # Errors
///
/// Returns fail-closed validation, transport, or response-framing errors.
pub fn execute_project_history_retrieval_cli(
    invocation: &ProjectHistoryRetrievalCliInvocation,
) -> Result<NaruonLiveResponse, ApiError> {
...
///
/// Evidence text and findings belong to the stored projection and are admitted.
...
/// `tepp.scientific_acceptance.v1`, a non-association inference status, or a
/// success body that is not a stored `ProjectHistoryProjection`.
pub fn render_project_history_retrieval_cli_stdout(
    invocation: &ProjectHistoryRetrievalCliInvocation,
...
/// wire limit.
pub fn read_project_history_retrieval_cli_stdin(
