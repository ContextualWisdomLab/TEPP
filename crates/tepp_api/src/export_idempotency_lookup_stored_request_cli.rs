...
use crate::export_idempotency_lookup_stored_request_http::export_idempotency_lookup_stored_request_path_key;
...
use crate::{
    ...
    EXPORT_IDEMPOTENCY_LOOKUP_KEY_MAX_LEN, ErrorEnvelope, ExportAuthorizationRequest,
...
pub enum ExportIdempotencyLookupStoredRequestCliVerb {
    /// `GET /v1/exports/by-idempotency/{idempotency_key}/request`.
    Get,
}

impl ExportIdempotencyLookupStoredRequestCliVerb {
...
/// One operator CLI invocation against a loopback export lookup stored-request listener.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ExportIdempotencyLookupStoredRequestCliInvocation {
    /// CLI verb to execute.
    pub verb: ExportIdempotencyLookupStoredRequestCliVerb,
...
impl ExportIdempotencyLookupStoredRequestCliInvocation {
        ...
        S: AsRef<str>,
    {
        ...
        let verb = ExportIdempotencyLookupStoredRequestCliVerb::parse(verb_token)?;
    ...
    pub fn validate(&self) -> Result<(), ApiError> {
        ...
        if self.idempotency_key == crate::EXPORT_IDEMPOTENCY_LOOKUP_PREFIX
        ...
        refuse_metrics_on_export_lookup_stored_request_payload(&self.body)?;
        Ok(())
    }
}
...
fn assemble_invocation(
    verb: ExportIdempotencyLookupStoredRequestCliVerb,
    flags: ParsedFlags,
    body: String,
) -> Result<ExportIdempotencyLookupStoredRequestCliInvocation, ApiError> {
    let invocation = ExportIdempotencyLookupStoredRequestCliInvocation {
        verb,
...
pub fn loopback_http1_from_export_idempotency_lookup_stored_request_exchange(
...
pub fn compose_export_idempotency_lookup_stored_request_cli_http(
    ...
    loopback_http1_from_export_idempotency_lookup_stored_request_exchange(
        &exchange,
        &invocation.host,
    )
}
...
pub fn dispatch_export_idempotency_lookup_stored_request_cli(
    service: &mut AnalysisRunLiveService,
    invocation: &ExportIdempotencyLookupStoredRequestCliInvocation,
) -> Result<NaruonLiveResponse, ApiError> {
    let request = compose_export_idempotency_lookup_stored_request_cli_http(invocation)?;
...
pub fn execute_export_idempotency_lookup_stored_request_cli(
    invocation: &ExportIdempotencyLookupStoredRequestCliInvocation,
) -> Result<NaruonLiveResponse, ApiError> {
...
pub fn render_export_idempotency_lookup_stored_request_cli_stdout(
    ...
    refuse_metrics_on_export_lookup_stored_request_payload(&response.body)?;
    ...
    let stored = serde_json::from_str::<ExportAuthorizationRequest>(&response.body)
        .map_err(|_| ApiError::InvalidWirePayload)?;
...
/// limit.
pub fn read_export_idempotency_lookup_stored_request_cli_stdin(
...
mod branch_coverage_tests {
    ...
    use super::{
        ExportIdempotencyLookupStoredRequestCliInvocation,
        ExportIdempotencyLookupStoredRequestCliVerb, parse_http_response,
        read_export_idempotency_lookup_stored_request_cli_stdin, valid_http_field_name,
    };
    use crate::{
    ...
    fn invocation() -> ExportIdempotencyLookupStoredRequestCliInvocation {
        ExportIdempotencyLookupStoredRequestCliInvocation {
            verb: ExportIdempotencyLookupStoredRequestCliVerb::Get,
    ...
    fn invocation_and_flag_error_arms_are_covered() {
        ...
        assert!(super::compose_export_idempotency_lookup_stored_request_cli_http(&value).is_err());
        ...
        for args in [
            ...
            assert!(
                ExportIdempotencyLookupStoredRequestCliInvocation::from_args(args, "").is_err()
            );
        }
        let valid = invocation();
        assert_eq!(
            super::compose_export_idempotency_lookup_stored_request_cli_http(&valid),
            Err(ApiError::AuthorizationDenied)
        );
        assert_eq!(
            naruon_export_idempotency_lookup_stored_request_exchange(
    ...
    fn response_parser_and_reader_error_arms_are_covered() {
        ...
        assert!(
            read_export_idempotency_lookup_stored_request_cli_stdin(false, Cursor::new([0xff]))
                .is_err()
        );
        assert!(
        ...
        assert!(
            read_export_idempotency_lookup_stored_request_cli_stdin(false, FailingReader).is_err()
        );
