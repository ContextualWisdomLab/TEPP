//! Operator loopback CLI for naruon export stored-request GET.
//!
//! GAP-003A unique slice: operators run `tepp-export-request get` to mint
//! `naruon_export_stored_request_exchange` onto spawned `tepp-loopback` TCP.
//! Stdout is the stored `ExportAuthorizationRequest`.
...
//! stored-request GET (#457), GET-by-id HTTP (#411), retrieval CLI (#417),
//! collection GET/CLI (#443/#444), export-authorize CLI (#410),
//! project-history stored-request CLI (#456), interpretation-run
//! stored-request CLI (#454), cancel lineages (closed), Leiden, or GAP-010
...
use crate::{
    ...
    ExportAuthorizationRequest, NARUON_CONSUMER_CODE, NARUON_LIVE_HEADER_BYTE_LIMIT,
...
/// Supported operator verbs for the loopback export stored-request CLI.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ExportStoredRequestCliVerb {
    /// `GET /v1/exports/{export_id}/request`.
    Get,
}

impl ExportStoredRequestCliVerb {
...
/// One operator CLI invocation against a loopback export stored-request listener.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ExportStoredRequestCliInvocation {
    /// CLI verb to execute.
    pub verb: ExportStoredRequestCliVerb,
    /// Loopback `host:port` of `tepp-loopback`.
    pub host: String,
    /// Published HTTPS origin used to mint the typed stored-request exchange.
    pub origin: String,
    /// Published modular consumer. Stored-request GET admits `naruon` only.
    pub consumer: String,
    /// Opaque export identity minted by naruon export authorization.
    pub export_id: String,
    /// JSON body. Stored-request GET requires empty.
    pub body: String,
}

impl ExportStoredRequestCliInvocation {
        ...
        S: AsRef<str>,
    {
        ...
        let verb = ExportStoredRequestCliVerb::parse(verb_token)?;
    ...
    pub fn validate(&self) -> Result<(), ApiError> {
        ...
        if self.export_id.len() > crate::EXPORT_RETRIEVAL_ID_MAX_LEN {
            return Err(ApiError::LimitExceeded);
        }
        if !self.body.is_empty() {
            return Err(ApiError::InvalidWirePayload);
        }
        refuse_metrics_on_export_stored_request_payload(&self.body)?;
        Ok(())
    }
}
...
fn assemble_invocation(
    verb: ExportStoredRequestCliVerb,
    flags: ParsedFlags,
    body: String,
) -> Result<ExportStoredRequestCliInvocation, ApiError> {
    let invocation = ExportStoredRequestCliInvocation {
        verb,
...
/// collection, cancel extra-segments, and pagination headers fail closed.
///
/// # Errors
///
...
/// exchange is not a GET `/v1/exports/{export_id}/request` with an empty body.
pub fn loopback_http1_from_export_stored_request_exchange(
    ...
    let path = rest
        ...
        .ok_or(ApiError::InvalidWirePayload)?;
    let _export_id = export_stored_request_path_id(path)?;
...
/// Compose one HTTP/1.1 stored-request GET from the typed naruon exchange.
///
/// # Errors
///
/// Returns the same fail-closed errors as
/// [`ExportStoredRequestCliInvocation::validate`].
pub fn compose_export_stored_request_cli_http(
    invocation: &ExportStoredRequestCliInvocation,
) -> Result<String, ApiError> {
    invocation.validate()?;
    let exchange =
        naruon_export_stored_request_exchange(&invocation.origin, &invocation.export_id)?;
    loopback_http1_from_export_stored_request_exchange(&exchange, &invocation.host)
}
...
pub fn dispatch_export_stored_request_cli(
    service: &mut AnalysisRunLiveService,
    invocation: &ExportStoredRequestCliInvocation,
) -> Result<NaruonLiveResponse, ApiError> {
    let request = compose_export_stored_request_cli_http(invocation)?;
...
pub fn execute_export_stored_request_cli(
    invocation: &ExportStoredRequestCliInvocation,
) -> Result<NaruonLiveResponse, ApiError> {
...
///
/// Tenant, principal, and `includes_source_text` belong to the stored
/// authorization request and are admitted. RMSE, bias, coverage, SE-gate, and
/// causal-score keys fail closed.
///
/// # Errors
///
...
pub fn render_export_stored_request_cli_stdout(
    invocation: &ExportStoredRequestCliInvocation,
    ...
    refuse_metrics_on_export_stored_request_payload(&response.body)?;
    ...
    let stored: ExportAuthorizationRequest = crate::wire::from_json(&response.body)?;
    crate::wire::to_json(&stored)
}
...
/// limit.
pub fn read_export_stored_request_cli_stdin(
...
mod branch_coverage_tests {
    ...
    use super::{
        ExportStoredRequestCliInvocation, ExportStoredRequestCliVerb,
        loopback_http1_from_export_stored_request_exchange, parse_http_response,
        read_export_stored_request_cli_stdin, valid_http_field_name,
    };
    use crate::{
        ApiError, DEFAULT_PROJECT_HISTORY_BYTE_LIMIT, NARUON_CONSUMER_CODE,
        naruon_export_stored_request_exchange,
    };

    fn invocation() -> ExportStoredRequestCliInvocation {
        ExportStoredRequestCliInvocation {
            verb: ExportStoredRequestCliVerb::Get,
    ...
    fn invocation_and_flag_error_arms_are_covered() {
        ...
        assert_eq!(value.validate(), Err(ApiError::InvalidWirePayload));
        value = invocation();
        value.export_id = "export\nother".into();
        assert_eq!(value.validate(), Err(ApiError::InvalidWirePayload));
        value = invocation();
        value.origin = "https://bad/path".into();
        assert!(super::compose_export_stored_request_cli_http(&value).is_err());
        ...
        for args in [
            ...
            assert!(ExportStoredRequestCliInvocation::from_args(args, "").is_err());
        }
    }
    ...
    fn exchange_header_and_target_error_arms_are_covered() {
        ...
        let base = naruon_export_stored_request_exchange(origin, "export-1").expect("exchange");
        let mut cases = Vec::new();
        let mut value = base.clone();
        value.body = "{}".into();
        cases.push(value);
        let mut value = base.clone();
        value.target_url = "http://tepp.example.test/v1/exports/export-1".into();
        cases.push(value);
        let mut value = base.clone();
        ...
        for value in cases {
            assert!(
                loopback_http1_from_export_stored_request_exchange(&value, "127.0.0.1:18081")
                    .is_err()
            );
        }
    }
    ...
    fn response_parser_and_reader_error_arms_are_covered() {
        ...
        assert!(read_export_stored_request_cli_stdin(false, Cursor::new([0xff])).is_err());
        assert!(
            read_export_stored_request_cli_stdin(
                false,
                Cursor::new(vec![b'a'; DEFAULT_PROJECT_HISTORY_BYTE_LIMIT + 1]),
            )
            .is_err()
        );
        assert!(read_export_stored_request_cli_stdin(false, FailingReader).is_err());
