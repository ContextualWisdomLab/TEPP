//! Operator loopback CLI for analysis-run retry-parent GET.
//!
//! GAP-003A retry-parent CLI slice: operators run `tepp-retry-parent parent`
//! to mint a typed naruon/`LineageWeave` retry-parent GET onto spawned
//! `tepp-loopback` TCP. Stdout is a metric-free `200 OK` inspect
//! (`"parent": null` when the run was never retried).
...
//! Naruon-only compatibility inspect already owned by ADR 0044.
...
use crate::{
    lineageweave_analysis_run_retry_parent_exchange, naruon_analysis_run_retry_parent_exchange,
    refuse_metrics_on_retry_parent_payload, AnalysisRunLiveService, AnalysisRunRetryParent,
    ApiError, NaruonHttpExchange, NaruonLiveResponse, ANALYSIS_RUN_RETRY_PARENT_ID_MAX_LEN,
    LINEAGEWEAVE_CONSUMER_CODE, NARUON_CONSUMER_CODE, NARUON_LIVE_IO_TIMEOUT,
};

const SCIENTIFIC_ACCEPTANCE_SCHEMA: &str = "tepp.scientific_acceptance.v1";

/// Supported operator verbs for the loopback retry-parent CLI.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum AnalysisRunRetryParentCliVerb {
    /// `GET /v1/analysis-runs/{run_id}/parent`.
    Parent,
}

impl AnalysisRunRetryParentCliVerb {
...
/// One operator CLI invocation against a loopback retry-parent GET listener.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct AnalysisRunRetryParentCliInvocation {
    /// CLI verb to execute.
    pub verb: AnalysisRunRetryParentCliVerb,
    /// Loopback `host:port` of `tepp-loopback`.
    pub host: String,
    /// Published HTTPS origin used to mint the typed retry-parent exchange.
    pub origin: String,
    /// Published modular consumer (`naruon` or `lineageweave`).
    pub consumer: String,
    /// Opaque server-assigned run identity to inspect.
    pub run_id: String,
    /// JSON body. Retry-parent GET requires empty.
    pub body: String,
}

impl AnalysisRunRetryParentCliInvocation {
    /// Parse argv plus stdin body into a validated loopback retry-parent invocation.
    ///
    /// # Errors
    ///
        ...
        S: AsRef<str>,
    {
        ...
        let verb = AnalysisRunRetryParentCliVerb::parse(verb_token)?;
    ...
    pub fn validate(&self) -> Result<(), ApiError> {
        ...
        refuse_metrics_on_retry_parent_payload(&self.body)?;
        Ok(())
    }
}
...
fn assemble_invocation(
    ...
    flags: ParsedFlags,
    body: String,
) -> Result<AnalysisRunRetryParentCliInvocation, ApiError> {
    let invocation = AnalysisRunRetryParentCliInvocation {
        verb,
...
fn retry_parent_exchange(
    invocation: &AnalysisRunRetryParentCliInvocation,
) -> Result<NaruonHttpExchange, ApiError> {
    if invocation.consumer == LINEAGEWEAVE_CONSUMER_CODE {
        lineageweave_analysis_run_retry_parent_exchange(&invocation.origin, &invocation.run_id)
    } else if invocation.consumer == NARUON_CONSUMER_CODE {
        naruon_analysis_run_retry_parent_exchange(&invocation.origin, &invocation.run_id)
    } else {
        Err(ApiError::InvalidWirePayload)
    }
}

/// Render a typed retry-parent exchange as HTTP/1.1 for a bound loopback listener.
///
...
/// exchange is not a GET `/parent`.
pub fn loopback_http1_from_retry_parent_exchange(
    ...
    let path = rest
        ...
        .ok_or(ApiError::InvalidWirePayload)?;
    if path.rsplit('/').next() != Some("parent") {
        return Err(ApiError::InvalidWirePayload);
    }
...
/// Compose one HTTP/1.1 retry-parent GET from the typed consumer exchange.
///
/// # Errors
///
/// Returns the same fail-closed errors as
/// [`AnalysisRunRetryParentCliInvocation::validate`].
pub fn compose_analysis_run_retry_parent_cli_http(
    invocation: &AnalysisRunRetryParentCliInvocation,
) -> Result<String, ApiError> {
    invocation.validate()?;
    let exchange = retry_parent_exchange(invocation)?;
    loopback_http1_from_retry_parent_exchange(&exchange, &invocation.host)
}

/// Dispatch one retry-parent CLI invocation against an in-process loopback service.
///
/// # Errors
///
/// Returns fail-closed validation errors before the HTTP handler runs.
pub fn dispatch_analysis_run_retry_parent_cli(
    service: &mut AnalysisRunLiveService,
    invocation: &AnalysisRunRetryParentCliInvocation,
) -> Result<NaruonLiveResponse, ApiError> {
    let request = compose_analysis_run_retry_parent_cli_http(invocation)?;
    Ok(service.handle_http_request(&request))
}

/// Execute one retry-parent CLI invocation over loopback TCP against `tepp-loopback`.
///
/// # Errors
///
/// Returns fail-closed validation, transport, or response-framing errors.
pub fn execute_analysis_run_retry_parent_cli(
    invocation: &AnalysisRunRetryParentCliInvocation,
) -> Result<NaruonLiveResponse, ApiError> {
...
/// Filter CLI stdout so retry-parent inspect never prints scientific acceptance.
///
/// # Errors
///
...
/// retry-parent inspect for the requested `run_id`.
pub fn render_analysis_run_retry_parent_cli_stdout(
    invocation: &AnalysisRunRetryParentCliInvocation,
    ...
    refuse_metrics_on_retry_parent_payload(&response.body)?;
    ...
    let inspected = AnalysisRunRetryParent::from_json(&response.body)?;
...
/// Read stdin leftover bytes on a non-terminal; retry-parent GET requires empty.
///
/// # Errors
///
/// Returns [`ApiError::InvalidWirePayload`] when stdin cannot be read.
pub fn read_analysis_run_retry_parent_cli_stdin(
