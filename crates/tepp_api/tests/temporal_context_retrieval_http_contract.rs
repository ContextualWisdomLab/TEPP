//! Contract tests for loopback `GET /v1/temporal-context/{idempotency_key}`.

use std::io::{Read, Write};

use tepp_api::{
    AnalysisRunLiveService, LINEAGEWEAVE_CONSUMER_CODE, NARUON_CONSUMER_CODE, TEMPORAL_CONTEXT_PATH,
...
#[test]
fn get_by_id_returns_metric_free_identity() {
    let mut service = AnalysisRunLiveService::new();
    assert_eq!(
        service.handle_http_request(&post_http("idem-b")).status_code,
        200
    );
    let got = service.handle_http_request(
        &format!(
            "GET {TEMPORAL_CONTEXT_PATH}/idem-b HTTP/1.1\r\nHost: 127.0.0.1\r\ncontent-type: application/json\r\ntepp-consumer: {LINEAGEWEAVE_CONSUMER_CODE}\r\ntepp-contract-version: 1\r\ncontent-length: 0\r\n\r\n"
        ),
    );
    assert_eq!(got.status_code, 200, "{}", got.body);
    let row = TemporalContextRetrieved::from_json(&got.body).expect("row");
    assert_eq!(row.idempotency_key, "idem-b");
    assert!(!got.body.contains("event_label"));
    assert!(!got.body.contains("actor_references"));
    assert!(!got.body.contains("tepp.scientific_acceptance.v1"));
}

#[test]
fn get_by_id_refuses_naruon_and_serves_over_tcp() {
    ...
    assert_eq!(
        service
            .handle_http_request(
                &format!(
                    "GET {TEMPORAL_CONTEXT_PATH}/idem-tcp HTTP/1.1\r\nHost: 127.0.0.1\r\ncontent-type: application/json\r\ntepp-consumer: {NARUON_CONSUMER_CODE}\r\ntepp-contract-version: 1\r\ncontent-length: 0\r\n\r\n"
                )
            )
            .status_code,
        400
    ...
    assert!(!text.contains("event_label"), "{text}");
