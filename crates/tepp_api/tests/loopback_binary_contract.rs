...
#[test]
fn binary_inspects_a_retry_parent_over_tcp_after_cancel_and_retry() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_tepp-loopback"))
        .args(["127.0.0.1:0", "4"])
        .stdout(Stdio::piped())
        .spawn()
    ...
    let body = r#"{"contract_version":1,"idempotency_key":"loopback-parent-idem","tenant_workspace_id":"loopback-parent-tenant","snapshot_id":"loopback-parent-snapshot","knowledge_cutoff":"2026-08-01T00:00:00Z","model_contract_version":"tepp-analysis-run-v1","output_profile":"calibrated_event_measurement"}"#;
    let create = format!(
    ...
    let cancel_body = format!(
        r#"{{"contract_version":1,"run_id":"{run_id}","idempotency_key":"loopback-parent-idem"}}"#
    );
    let cancel = format!(
    ...
    let retry_body = format!(
        r#"{{"contract_version":1,"run_id":"{run_id}","idempotency_key":"loopback-parent-retry-idem"}}"#
    );
    let retry = format!(
    ...
    let inspect = format!(
        "GET /v1/analysis-runs/{child_id}/parent HTTP/1.1\r\nHost: {host}\r\ncontent-type: application/json\r\ntepp-consumer: lineageweave\r\ntepp-contract-version: 1\r\ncontent-length: 0\r\n\r\n"
    );
    let mut stream = TcpStream::connect(host).expect("connect parent");
    ...
    assert!(inspected.contains("\"parent\":{") || inspected.contains("\"parent\": {"));
    assert!(!inspected.contains("\"parent\":null"));
    ...
    assert!(!inspected.contains("retried_from"));
    assert!(child.wait().expect("wait").success());
}
