...
#[test]
fn binary_cancels_an_accepted_analysis_run_over_tcp() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_tepp-loopback"))
        .args(["127.0.0.1:0", "2"])
        .stdout(Stdio::piped())
        .spawn()
    ...
    let body = r#"{"contract_version":1,"idempotency_key":"loopback-cancel-idem","tenant_workspace_id":"loopback-cancel-tenant","snapshot_id":"loopback-cancel-snapshot","knowledge_cutoff":"2026-08-01T00:00:00Z","model_contract_version":"tepp-analysis-run-v1","output_profile":"calibrated_event_measurement"}"#;
    let create = format!(
    ...
    assert!(!cancelled.contains("scientific_acceptance"));
    assert!(child.wait().expect("wait").success());
}
