...
#[test]
fn binary_lists_an_accepted_analysis_run_collection_over_tcp() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_tepp-loopback"))
        .args(["127.0.0.1:0", "2"])
        .stdout(Stdio::piped())
        .spawn()
    ...
    let body = r#"{"contract_version":1,"idempotency_key":"loopback-collection-idem","tenant_workspace_id":"loopback-collection-tenant","snapshot_id":"loopback-collection-snapshot","knowledge_cutoff":"2026-08-01T00:00:00Z","model_contract_version":"tepp-analysis-run-v1","output_profile":"calibrated_event_measurement"}"#;
    let create = format!(
    ...
    let mut stream = TcpStream::connect(host).expect("connect collection");
    stream.write_all(list.as_bytes()).expect("list");
    let mut listed = String::new();
    stream.read_to_string(&mut listed).expect("listed");
    assert!(listed.starts_with("HTTP/1.1 200 OK"));
    assert!(listed.contains(&format!("\"run_id\":\"{run_id}\"")));
    assert!(listed.contains("\"run_state\":\"accepted\""));
    assert!(listed.contains("\"idempotency_key\":\"loopback-collection-idem\""));
    assert!(!listed.contains("rmse"));
    assert!(!listed.contains("scientific_acceptance"));
    assert!(!listed.contains("terminal_result"));
    assert!(!listed.contains("tenant_workspace_id"));
    assert!(child.wait().expect("wait").success());
}
