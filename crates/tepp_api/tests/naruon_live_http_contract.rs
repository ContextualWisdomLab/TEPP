...
use tepp_api::{
    ...
    AnalysisRunRetryLineage, AnalyticalPurpose, ApiError, DEFAULT_ANALYSIS_RUN_BYTE_LIMIT,
...
#[test]
fn handle_http_lists_naruon_retry_lineage_and_refuses_lineageweave() {
    let mut service = NaruonLiveService::new();
    let run = sample_run();
    ...
    let inspect = http_request(
        "GET",
        &format!("{NARUON_ANALYSIS_RUN_PATH}/{}/retries", accepted.run_id),
        &[
            ("Host".into(), "127.0.0.1".into()),
            ("content-type".into(), "application/json".into()),
            ("tepp-consumer".into(), "naruon".into()),
