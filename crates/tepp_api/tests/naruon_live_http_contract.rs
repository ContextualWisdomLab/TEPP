...
use tepp_api::{
    ...
    AnalysisRunStoredRequest, AnalyticalPurpose, ApiError, DEFAULT_ANALYSIS_RUN_BYTE_LIMIT,
...
#[test]
fn handle_http_inspects_naruon_stored_request_and_refuses_lineageweave() {
    let mut service = NaruonLiveService::new();
    let run = sample_run();
    ...
    let inspect = http_request(
        "GET",
        &format!("{NARUON_ANALYSIS_RUN_PATH}/{}/request", accepted.run_id),
        &[
            ("Host".into(), "127.0.0.1".into()),
            ("content-type".into(), "application/json".into()),
            ("tepp-consumer".into(), "naruon".into()),
