...
/// Prompt-source artifact and execution contracts from this engine.
pub use prompt_source_artifact::{
    PROMPT_SOURCE_ARTIFACT_BYTE_LIMIT, PROMPT_SOURCE_ARTIFACT_SCHEMA_VERSION,
    PROMPT_SOURCE_MODEL_CONTRACT_VERSION, PROMPT_SOURCE_OUTPUT_PROFILE, PromptSourceArtifact,
    PromptSourceDocument, PromptSourceExecution, execute_prompt_source_run,
};
...
pub enum AnalysisEngineError {
    ...
    /// A prompt-source artifact violated its bounded schema or count invariants.
    InvalidPromptSourceArtifact,
}
...
impl fmt::Display for AnalysisEngineError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        let message = match self {
            ...
            Self::InvalidPromptSourceArtifact => "invalid prompt-source artifact",
        };
...
mod tests {
    ...
    fn public_accessors_limits_and_error_messages_are_executable() {
        ...
        let messages = [
            ...
            (
                AnalysisEngineError::InvalidPromptSourceArtifact,
                "invalid prompt-source artifact",
            ),
        ];
