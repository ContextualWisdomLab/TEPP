//! Digest-bound episode-membership refusals as an analysis-run profile.

use episode_membership::{EpisodeMembershipError, EventWindow, refuse_membership_outside_episode};
...
/// Versioned schema for a completed episode-membership artifact.
pub const EPISODE_MEMBERSHIP_ARTIFACT_SCHEMA_VERSION: &str = "tepp.episode_membership.v1";
/// Model contract required by the episode-membership execution path.
...
/// One cutoff-admitted membership assignment against an episode window.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EpisodeMembershipAssignment {
...
impl EpisodeMembershipAssignment {
    /// Construct a bounded episode-membership assignment.
    ///
    /// # Errors
    ///
    ...
    /// Return the membership event-time window.
    #[must_use]
    pub const fn membership(&self) -> EventWindow {
        self.membership
    }

    /// Return the episode event-time window.
    #[must_use]
    pub const fn episode(&self) -> EventWindow {
        self.episode
    }
...
/// Completed, bounded episode-membership census for analysis-run clients.
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct EpisodeMembershipArtifact {
    ...
    /// Assignments contained in their episode window.
    pub contained_count: u64,
    /// Assignments that escaped their episode window.
    pub escaped_count: u64,
    /// Escaped assignments refused as membership-outside-episode.
    pub refused_as_escape_count: u64,
    /// Fixed claim boundary for consumer copy.
    pub inference_status: String,
}

impl EpisodeMembershipArtifact {
    /// Parse and fully validate a bounded artifact JSON payload.
    ///
    /// # Errors
    ///
    /// Returns [`AnalysisEngineError::InvalidEpisodeMembershipArtifact`] when
    ...
    pub fn from_json(payload: &str) -> Result<Self, AnalysisEngineError> {
        ...
        let artifact: Self = serde_json::from_str(payload)
            .map_err(|_| AnalysisEngineError::InvalidEpisodeMembershipArtifact)?;
        artifact.validate()?;
        Ok(artifact)
    }
    ...
    fn validate(&self) -> Result<(), AnalysisEngineError> {
        ...
        if self.schema_version != EPISODE_MEMBERSHIP_ARTIFACT_SCHEMA_VERSION
            ...
            || self.inference_status != EPISODE_MEMBERSHIP_INFERENCE_STATUS
        {
            return Err(AnalysisEngineError::InvalidEpisodeMembershipArtifact);
        }
        Ok(())
    }
}

/// One completed episode-membership artifact and its terminal result.
#[derive(Clone, Debug, PartialEq)]
pub struct EpisodeMembershipExecution {
    /// Digest-bound completed episode-membership census.
    pub artifact: EpisodeMembershipArtifact,
    /// Terminal result carrying the artifact identity, digest, and schema.
    pub terminal_result: AnalysisRunTerminalResult,
}

/// Execute cutoff-safe episode-membership refusals as one analysis-run profile.
///
/// The executor invokes [`refuse_membership_outside_episode`] already on
/// protected main. Contained windows stay membership. Escaped windows stay
/// refusals, never subevent-versus-parent containment. It does not emit
...
/// single-class corpus, inverted or escaped membership treated as success,
/// duplicate assignment identity, oversized corpus, or invalid artifact
/// error.
pub fn execute_episode_membership_run(
...
) -> Result<EpisodeMembershipExecution, AnalysisEngineError> {
    request.to_json()?;
    accepted.to_json()?;
    ...
    let terminal_result = AnalysisRunTerminalResult::succeeded(
        request,
        accepted,
        format!("episode_membership_artifact_{}", &digest[..16]),
        digest,
        EPISODE_MEMBERSHIP_ARTIFACT_SCHEMA_VERSION,
        completed_at,
        summary,
    )?;
    Ok(EpisodeMembershipExecution {
        artifact,
        terminal_result,
    })
}
...
mod tests {
    ...
    fn assert_invalid(artifact: &EpisodeMembershipArtifact) {
        assert_eq!(
            artifact.to_json(),
    ...
    fn artifact_round_trip_and_size_bounds_fail_closed() {
        ...
        assert_eq!(
            EpisodeMembershipArtifact::from_json(&payload),
            Ok(artifact.clone())
        );
        assert_eq!(artifact.sha256().expect("digest").len(), 64);
        assert_eq!(
            EpisodeMembershipArtifact::from_json("{}"),
            Err(AnalysisEngineError::InvalidEpisodeMembershipArtifact)
        );
        assert_eq!(
            EpisodeMembershipArtifact::from_json(
                &"x".repeat(EPISODE_MEMBERSHIP_ARTIFACT_BYTE_LIMIT + 1)
            ),
