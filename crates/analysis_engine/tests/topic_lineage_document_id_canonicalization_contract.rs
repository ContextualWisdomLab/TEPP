...
fn artifact(predecessor_document_id: &str) -> TopicLineageArtifact {
    TopicLineageArtifact {
        ...
        sequence_edges: vec![TopicLineageArtifactEdge {
            predecessor_document_id: predecessor_document_id.into(),
            successor_document_id: "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb".into(),
            topic_index: 0,
...
#[test]
fn artifact_document_ids_require_one_canonical_uuid_byte_identity() {
    let canonical = artifact("aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa");
    assert!(canonical.to_json().is_ok());

    let simple_alias = artifact("aaaaaaaaaaaa4aaa8aaaaaaaaaaaaaaa");
    assert!(
        simple_alias.to_json().is_err(),
        "equivalent UUID text must not mint a second valid artifact byte identity"
    );
}
