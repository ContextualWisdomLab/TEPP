...
#[test]
fn restrictive_policy_requires_permissive_coverage_for_the_same_role() {
    let catalog = catalog_with_policies(&format!(
        r"
CREATE POLICY document_record_writer_isolation ON document_record
    AS PERMISSIVE
    FOR SELECT
    TO writer_role
    USING ({TENANT_BINDING});
...
#[test]
fn restrictive_policy_may_narrow_matching_role_access() {
    let catalog = catalog_with_policies(&format!(
        r"
