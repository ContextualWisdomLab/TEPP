...
#[test]
fn postgresql_identifier_continuations_cannot_donate_rls_enable_evidence() {
    for alter_target in ["document_record$shadow", "document_record측정"] {
        let up_sql = format!(
            r"
            CREATE TABLE document_record (
                document_record_id uuid PRIMARY KEY,
            ...
            CREATE ROLE tepp_app_runtime NOSUPERUSER NOBYPASSRLS;
            ALTER TABLE {alter_target} ENABLE ROW LEVEL SECURITY;
            ALTER TABLE {alter_target} FORCE ROW LEVEL SECURITY;
        ...
        assert_eq!(
            ...
            "RLS enable target prefix was accepted for {alter_target}"
        );
    }
}
