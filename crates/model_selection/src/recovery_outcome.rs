...
//!
//! Recovery summaries must retain numerical fit/selection/state-recovery failures
//! in the attempted denominator without turning structural experiment invalidity
//! into an ordinary failed replication. This module owns that distinction so
//! repeated recovery harnesses do not reimplement it with `.ok()` or caller-local
//! error whitelists.
...
/// unusable numerical model-selection diagnostics, or numerical held-out
/// document-state recovery become `None`, preserving the attempted replication
/// without fabricating a selected result or recovery metric. All other errors
/// propagate because they describe an invalid candidate design, split, identity,
/// chronology, authority boundary, or recovery composition rather than a
/// stochastic/numerical replication failure.
///
/// The function is generic so the same owner classification can be applied to
/// complete-grid fitting, selected-`K` results, and #720 held-out state recovery.
/// It must not be used to suppress errors from unrelated workflows.
///
/// # Errors
///
...
/// [`ModelSelectionError::InvalidDiagnostic`], and
/// [`ModelSelectionError::HeldOutStateRecoveryFailed`], which are represented as
/// a failed recovery replication (`Ok(None)`).
