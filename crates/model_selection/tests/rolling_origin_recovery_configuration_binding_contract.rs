//! Scientific recovery must bind surviving fits to the predeclared numerical design.
...
fn fit(
    ...
    let config = ReferenceTopicModelConfig::new(topic_count, seeds, 2_000, 0.001)
...
#[test]
fn recovery_rejects_dimension_compatible_fits_from_a_different_numerical_design() {
    ...
    let ordered_ids: Vec<_> = training_ids
        .iter()
        .copied()
        .chain(evaluation_ids)
        .collect();
    let ordered_days = [1_u8, 2, 3, 4, 5, 6, 15, 16];
    ...
    let candidates = [
        fit(&training_input, 2, vec![7, 11, 19]),
        fit(&training_input, 3, vec![13, 17, 23]),
    ];
    ...
    let evaluation = RollingOriginRecoveryEvaluation::new(
        &partition,
        &candidates,
        &evaluation_ids,
        &evaluation_counts,
    ...
    assert_eq!(
        ...
        Err(ModelSelectionError::PredictiveCandidateConfigurationMismatch)
    );
}
