//! Scientific recovery must not silently shrink its predeclared candidate-K design.
...
#[test]
fn recovery_rejects_a_shared_survivor_set_that_omits_declared_k() {
    ...
    let candidates = [fit(&training_input, 2), fit(&training_input, 3)];
    ...
    let complete_grid =
        FittedCandidateKConfig::new(vec![2, 3], vec![7, 11, 19], 2_000, 0.001)
            .expect("complete candidate grid");
    let selected = select_declared_rolling_origin_recovery_candidate_k(
        &complete_grid,
