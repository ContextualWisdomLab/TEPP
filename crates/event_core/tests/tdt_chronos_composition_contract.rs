...
#[test]
fn composition_recovers_known_truth_metrics_and_refuses_promotion() {
    ...
    // Three documentary units: original story, noisy duplicate, delayed revise boundary.
    let truth_segmentation =
        StorySegmentation::new(3, vec![false, true]).expect("truth segmentation");
    let recovered_segmentation =
    ...
    let links = vec![
        EventLinkPair::new(
            ...
            fixture.award_noisy.mention_id(),
        )
    ...
    let forecasts = vec![ChronosOccurrenceForecast::new(
    ...
    let mut composition = compose_event_intelligence(
        workflow_config(),
        recovered_segmentation.clone(),
        mentions,
        links.clone(),
        first_story_labels.clone(),
        track_assignments.clone(),
        schema_slots.clone(),
        forecasts.clone(),
    )
