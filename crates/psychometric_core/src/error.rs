...
pub enum PsychometricError {
    ...
    /// 2017-era active `if(asymptotes==TRUE) TIPREDEFFECT <- -DRIFT %*% TIPREDEFFECT`
    /// was requested for a non-stable drift. That rewrite is
    /// `−a · B` and converts the asymptotic-parameterization stored
    /// `TIPREDEFFECT` back to original `TIPREDEFFECT`. Lasting
    /// conversion requires `a < 0`. A zero coefficient is exactly
    /// zero even if `a ≥ 0`.
    AsymptotesTrueTimeIndependentEffectRequiresStableDrift,
    /// Unstandardised `TIPREDEFFECT` was treated as the 2017-era
    /// active `asymptotes=TRUE` rewrite. `B` is not `−a · B` when
    /// `a ≠ −1`.
    UnstandardisedTimeIndependentEffectIsNotAsymptotesTrueTimeIndependentEffect,
    /// Driver §7.2 `asymTIPREDEFFECT` `-B / a` was treated as the
    /// 2017-era active `asymptotes=TRUE` `TIPREDEFFECT` rewrite.
    /// The stored asymptotic effect is not `−a · B`.
    AsymptoticTimeIndependentEffectIsNotAsymptotesTrueTimeIndependentEffect,
    /// Driver Table 2 `discreteTIPREDEFFECT` `A^{-1}[e^{A Δt} − I] B`
    /// was treated as the 2017-era active `asymptotes=TRUE`
    /// `TIPREDEFFECT` rewrite. A finite event interval is not
    /// `−a · B`.
    DiscreteTimeIndependentEffectIsNotAsymptotesTrueTimeIndependentEffect,
    /// Driver p. 16 `TIPREDEFFECTstd` `B · √v / √p` was treated as
    /// the 2017-era active `asymptotes=TRUE` `TIPREDEFFECT` rewrite.
    /// The standardised coefficient is not `−a · B`.
    StandardisedTimeIndependentEffectIsNotAsymptotesTrueTimeIndependentEffect,
}
...
impl fmt::Display for PsychometricError {
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        let message = match self {
            ...
            Self::UnstandardisedTimeIndependentEffectIsNotAsymptotesTrueTimeIndependentEffect => {
                "unstandardised time-independent effect is not asymptotes-true time-independent effect"
            }
            ...
            Self::StandardisedTimeIndependentEffectIsNotAsymptotesTrueTimeIndependentEffect => {
                "standardised time-independent effect is not asymptotes-true time-independent effect"
            }
        };
