...
///
/// Driver, Oud, and Voelkle (2017, Eq. 5, p. 5; Eq. 3–4, pp. 4–5;
/// §4.3, pp. 9–10; Table 2, p. 12; p. 16; §7.2, pp. 20–21; JSS PDF
/// re-opened 2026-08-31T09:55Z from
/// <https://www.jstatsoft.org/index.php/jss/article/download/v077i05/1104>)
/// write `y_i(t) = Γ + Λ η_i(t) + ζ_i(t)` with `ζ ~ N(0, Θ)` and
/// `Γ ~ N(τ, Ψ)`. Section 4.3 treats the first time point as
/// predetermined when no assumptions are made about the process
/// prior to `T0`: the initial latent variance `p_0` is freely
/// estimated. Equation 3 writes `η(t) = exp(A Δt) η(t0) + … +` the
/// stochastic integral. Equation 4 writes that the integral exhibits
/// covariance `Q_Δt`. The law of total variance on the free first-
/// occasion state is `e^{2 a Δt} p_0 + Q_Δt`. Trait variance and
/// `addedTIPREDVAR` are time-invariant between-subject; they do not
/// enter that process-noise integral. The later-occasion latent
/// composition is `trait + e^{2 a Δt} p_0 + Q_Δt + (B / a)² v`.
/// Form the evolved free first-occasion variance first, then include
/// the trait, then include the TI extra variance, then add. Equation
/// 5 of that composition is
/// `Var(y_t) = λ²(trait + e^{2 a Δt} p_0 + Q_Δt + (B / a)² v) + θ + ψ`.
/// Form the predetermined later-occasion latent variance INLINE from
/// on-main recoveries, then `λ² p + θ + ψ`. A zero loading is
/// exactly `θ + ψ`. Setting `p_0 = −q / (2 a)` recovers the
/// stationary later-occasion observed variance. Stationary later
/// observed variance uses `−q / (2 a)` in place of free `p_0` and is
/// not this map when `p_0` is free. `MANIFESTVAR` `θ` is not this
/// composition. The predetermined later-occasion latent variance is
/// not this observed variance. As `Δt → ∞` with stable `a < 0` the
/// composition approaches contemporaneous stationary `Var(y_0)`.
/// As `Δt → 0+` the composition approaches
/// `λ²(trait + p_0 + (B / a)² v) + θ + ψ`. Nonzero diffusion with
/// `a ≥ 0` is a growing process and is kept. The interval must be
/// event time and strictly positive. This is not a Kalman filter,
/// not a matrix `expm`, and not ctsem estimation.
///
/// # Errors
///
...
/// [`recover_manifest_trait_plus_state_observed_variance`]. Returns
/// [`PsychometricError::EventTimeRequired`] for any non-event clock,
/// [`PsychometricError::NonPositiveInterval`] when `event_delta` is
/// not strictly positive,
/// [`PsychometricError::AsymptoticTimeIndependentEffectRequiresStableDrift`]
...
pub fn recover_predetermined_later_observed_variance(
    ...
    let later_latent = require_finite(trait_plus_evolved + added)?;
    recover_manifest_trait_plus_state_observed_variance(
        loading,
        later_latent,
        measurement_error_variance,
...
///
/// `λ²(trait + e^{2 a Δt}(−q / (2 a)) + Q_Δt + (B / a)² v) + θ + ψ`
/// uses `−q / (2 a)` in place of free `p_0`.
...
mod tests {
    ...
    fn predetermined_later_observed_variance_recovers_driver_equation_five_of_section_four_point_three(
        ...
        // λ²(trait + e^{2 a Δt} p_0 + Q_Δt + (B / a)² v) + θ + ψ.
        // Form the later latent INLINE from on-main recoveries.
        let printed_effect = -0.225_f64;
        let printed_asym = -1.673_f64;
        let log_rate = -printed_effect / printed_asym;
        let trait_variance = 1.0_f64;
        ...
        assert!(growing > measurement_error + manifest_trait);
    }
