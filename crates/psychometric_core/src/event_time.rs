...
/// `asymDIFFUSION`.
///
/// Driver, Oud, and Voelkle (2017, p. 16; Eq. 3–4, pp. 4–5;
/// footnote 4; Table 2, p. 12; §7.1, pp. 18–19; 2017-era ctsem
/// `summary.ctsemFit.R`; JSS PDF re-opened 2026-08-28T13:24Z from
/// <https://www.jstatsoft.org/index.php/jss/article/download/v077i05/1104>)
/// print discrete-time transformations for a chosen event interval
/// (`discreteDIFFUSION` is the covariance of the Eq. 4 stochastic
/// integral over that interval) and, when appropriate, standardised
/// matrices with the suffix `std`. The printed example on p. 16 is
/// `discreteDRIFTstd`, not `discreteDIFFUSIONstd`. Footnote 4:
/// standardisations use only the relevant variance, not the total.
/// Process noise is within-subject stochastic input, so that
/// relevant variance is within-subject `asymDIFFUSION`
/// `p = −q / (2 a)`, the same footnote 4 variance used for `DRIFT`.
/// The 2017-era `summary.ctsemFit.R` forms unstandardised
/// `discreteDIFFUSION` whenever `verbose = TRUE`, as
/// `matrix(solve(DRIFTHATCH) %*% (expm(DRIFTHATCH Δt) − I ⊗ I) %*%
/// rvectorize(DIFFUSION), nrow = n.latent)`. That source forms
/// `asymDIFFUSIONstd` and `discreteDRIFTstd` in the same
/// `verbose` block and does not form a `discreteDIFFUSIONstd`
/// matrix. The scalar map here is the footnote 4 standardisation of
/// that named discrete process noise: `Q_Δt / p`. Form strictly
/// positive `p` first, then `Q_Δt` from Equation 4, then the
/// ratio. In the scalar stationary case that ratio equals
/// `1 − exp(2 a Δt)` and does not depend on `q` once `q > 0`.
/// Unstandardised `Q_Δt` is defined for growing `a ≥ 0` and for
/// zero diffusion; standardised discrete `DIFFUSION` is not. Zero
/// `q` has no positive process SD and fails closed. Lasting
/// `asymDIFFUSION` requires stable `a < 0`. A non-event clock
/// fails closed. A non-positive event interval fails closed. The
/// continuous standardisation `q / p = −2 a` does not depend on
/// `Δt` and is not this finite-interval map. Section 7.1 warns
/// that omitting trait variance confounds between- and within-person
/// information. `Q_Δt / (trait + p + added)` uses the total, not
...
/// `discreteDIFFUSIONstd` does. This crate does not currently
/// export `DIFFUSIONstd`; the refuse still names that quantity.
///
/// # Errors
///
/// Always returns
...
/// Refuse treating Driver §7.1 trait-contaminated process-noise
/// ratio as p. 16 `discreteDIFFUSIONstd`.
///
/// `Q_Δt / (trait + p + added)` mixes between-subject `TRAITVAR`
/// into the process-noise ratio. Footnote 4 standardises
...
pub fn refuse_trait_contaminated_discrete_diffusion_as_standardised_discrete_diffusion(
    trait_contaminated_discrete_diffusion: f64,
    standardised_discrete_diffusion: f64,
) -> Result<f64, PsychometricError> {
    let _ = (
        trait_contaminated_discrete_diffusion,
        standardised_discrete_diffusion,
    );
    Err(PsychometricError::TraitContaminatedDiscreteDiffusionIsNotStandardisedDiscreteDiffusion)
}
...
mod tests {
    use super::{
        ...
        refuse_trait_contaminated_discrete_diffusion_as_standardised_discrete_diffusion,
    ...
    fn standardised_discrete_diffusion_recovers_driver_page_sixteen_after_positive_asymdiffusion() {
        ...
        // 2017-era summary.ctsemFit.R discreteDIFFUSION): form strictly
        // positive asymDIFFUSION = −q / (2 a), then Q_Δt / p. Scalar
        // stationary ratio is 1 − exp(2 a Δt). That source does not form
        // a discreteDIFFUSIONstd matrix.
        let diffusion = 0.4_f64;
        let log_rate = -0.5_f64;
        let event_delta = 1.0_f64;
