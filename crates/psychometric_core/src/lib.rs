...
//! immediately after `mxobj$TIPREDEFFECT$values`; form `a` first,
//! then negate, then multiply by `B`; a zero coefficient is exactly
//! zero even if `a ≥ 0`; `a ≥ 0` with a nonzero coefficient fails
//! closed; unstandardised `TIPREDEFFECT` is not that rewrite;
//! `asymTIPREDEFFECT` `-B / a` is not that rewrite; the finite-
//! interval increment is not that rewrite; `TIPREDEFFECTstd` is not
//! that rewrite; JSS PDF re-opened 2026-08-31T15:26Z),
//! and refuses
...
pub use event_time::recover_asymptotes_true_time_independent_effect;
...
/// Refuse treating §7.2 `asymTIPREDEFFECT` as the 2017-era active `asymptotes=TRUE` rewrite.
...
/// Refuse treating the finite-interval increment as the 2017-era active `asymptotes=TRUE` rewrite.
...
/// Refuse treating p. 16 `TIPREDEFFECTstd` as the 2017-era active `asymptotes=TRUE` rewrite.
pub use event_time::refuse_standardised_time_independent_effect_as_asymptotes_true_time_independent_effect;
...
pub use event_time::refuse_unstandardised_time_independent_effect_as_asymptotes_true_time_independent_effect;
