...
use psychometric_core::{
    ...
    ordinary_least_squares_slope, recover_asymptotes_true_time_independent_predictor_effect,
...
fn asymptotes_true_time_independent_effect_recovers_negated_rate_times_coefficient() {
    ...
    assert!(
        recovered_error < 1e-15,
        "Driver et al. (2017, 2017-era asymptotes=TRUE TIPREDEFFECT): RMSE {recovered_error} for -a times B = 0.2"
    );
    ...
    .expect("stored");
    let stored_error = rmse(&[effect], &[from_stored]);
    assert!(
        stored_error < 1e-15,
        "Driver et al. (2017): stored -B/a = 0.8 recovers original TIPREDEFFECT RMSE {stored_error}"
    );
    ...
    assert!(
        ...
        "Driver et al. (2017, §7.2): unit asymTIPREDEFFECT RMSE {asymptotic_error} must exceed rewrite RMSE {recovered_error}"
    );
    let discrete = recover_discrete_time_independent_predictor_effect(
        effect,
        1.0,
    ...
    assert!(
        discrete_error > 1e-3,
        "Driver et al. (2017, Table 2): discreteTIPREDEFFECT RMSE {discrete_error} must exceed rewrite RMSE {recovered_error}"
    );
    let unstandardised_error = rmse(&[effect], &[recovered]);
    assert!(
        unstandardised_error > 1e-3,
        "Driver et al. (2017, Table 2): unstandardised TIPREDEFFECT RMSE {unstandardised_error} must exceed rewrite RMSE {recovered_error}"
    );
    assert_eq!(
        recover_asymptotes_true_time_independent_predictor_effect(0.0, 0.5, LagClock::EventTime)
            .expect("zero")
            .to_bits(),
        0.0_f64.to_bits()
    );
    assert_eq!(
        recover_asymptotes_true_time_independent_predictor_effect(0.4, 0.5, LagClock::EventTime),
        Err(PsychometricError::AsymptotesTrueTimeIndependentEffectRequiresStableDrift)
    );
    assert_eq!(
        recover_asymptotes_true_time_independent_predictor_effect(0.4, -0.5, LagClock::SystemTime),
        Err(PsychometricError::EventTimeRequired)
    );
    assert_eq!(
        refuse_unstandardised_time_independent_coefficient_as_asymptotes_true_time_independent_effect(
        ...
        Err(PsychometricError::UnstandardisedTimeIndependentCoefficientIsNotAsymptotesTrueTimeIndependentEffect)
    );
    assert_eq!(
        refuse_asymptotic_time_independent_effect_as_asymptotes_true_time_independent_effect(
    ...
    assert_eq!(
        refuse_continuous_intercept_as_asymptotes_true_time_independent_effect(effect, recovered),
