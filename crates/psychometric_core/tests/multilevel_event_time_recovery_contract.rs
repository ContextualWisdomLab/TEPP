...
fn standardised_time_independent_predictor_variance_recovers_driver_table_three_correlation() {
    ...
    assert!(
        recovered_error < unstandardised_error,
        "Driver et al. (2017, Table 3): unstandardised TIPREDVAR RMSE {unstandardised_error} must exceed TIPREDVARstd RMSE {recovered_error}"
    );
    assert_eq!(
        refuse_unstandardised_time_independent_predictor_variance_as_standardised_time_independent_predictor_variance(
            predictor_variance,
...
fn standardised_time_independent_predictor_variance_refuses_non_event_clocks_and_does_not_keep_zero_variance()
 {
    assert_eq!(
        recover_standardised_time_independent_predictor_variance(0.4, LagClock::AssertionTime),
        Err(PsychometricError::EventTimeRequired)
    );
    assert_eq!(
        recover_standardised_time_independent_predictor_variance(0.4, LagClock::KnowledgeCutoff),
        Err(PsychometricError::EventTimeRequired)
    );
    assert_eq!(
...
#[test]
fn time_independent_predictor_variance_std_clock_path_is_runtime_opaque() {
    let clocks = [
        LagClock::SystemTime,
        LagClock::DocumentTime,
        LagClock::AssertionTime,
    ...
    let non_event = clocks[non_event_index];
    assert_eq!(
        recover_standardised_time_independent_predictor_variance(0.4, non_event),
        Err(PsychometricError::EventTimeRequired)
    );
}
