...
fn standardised_time_independent_effect_recovers_driver_page_sixteen_after_positive_p_and_v() {
    ...
    assert!(
        error < 1e-15,
        "Driver et al. (2017, p. 16 TIPREDEFFECTstd): RMSE {error} for B · √v / √p"
    );
    let unstandardised_rmse = (coefficient - expected).abs();
    assert!(
        error < unstandardised_rmse,
        "Driver et al. (2017, Table 2): unstandardised B RMSE {unstandardised_rmse} must exceed TIPREDEFFECTstd RMSE {error}"
    );
    ...
    let asymptotic_rmse = (asymptotic - expected).abs();
    assert!(
        error < asymptotic_rmse,
        "Driver et al. (2017, §7.2): asymTIPREDEFFECTstd RMSE {asymptotic_rmse} must exceed TIPREDEFFECTstd RMSE {error}"
    );
    let discrete = recover_discrete_time_independent_predictor_effect(
        coefficient,
        1.0,
    ...
    assert!(
        error < discrete_rmse,
        "Driver et al. (2017, p. 16): finite-interval standardised TIPREDEFFECT RMSE {discrete_rmse} must exceed TIPREDEFFECTstd RMSE {error}"
    );
    ...
    .expect("negative signed TIPREDEFFECTstd");
    assert!((negative + expected).abs() < 1e-15);
}

#[test]
fn standardised_time_independent_effect_refuses_non_event_clocks_and_does_not_keep_zero_q_or_v() {
    assert_eq!(
    ...
    assert_eq!(
        ...
        Err(PsychometricError::StandardisedTimeIndependentEffectRequiresPositivePredictorVariance)
    );
    assert_eq!(
        recover_standardised_time_independent_predictor_effect(
            0.2,
