...
fn standardised_asymptotic_time_independent_effect_recovers_driver_page_sixteen_after_positive_p_and_v()
 {
    ...
    let stationary =
        recover_stationary_latent_variance(diffusion, log_rate, LagClock::EventTime).expect("p");
    let expected = (-coefficient / log_rate) * predictor_variance.sqrt() / stationary.sqrt();
    let error = (recovered - expected).abs();
    assert!(
        error < 1e-15,
        "Driver et al. (2017, p. 16 asymTIPREDEFFECTstd): RMSE {error} for (−B / a) · √v / √p"
    );
    let unstandardised = -coefficient / log_rate;
    let unstandardised_rmse = (unstandardised - expected).abs();
    assert!(
        error < unstandardised_rmse,
        "Driver et al. (2017, §7.2): unstandardised −B / a RMSE {unstandardised_rmse} must exceed asymTIPREDEFFECTstd RMSE {error}"
    );
    let discrete = recover_discrete_time_independent_predictor_effect(
        coefficient,
        1.0,
    ...
    assert!(
        error < discrete_rmse,
        "Driver et al. (2017, p. 16): finite-interval standardised TIPREDEFFECT RMSE {discrete_rmse} must exceed asymTIPREDEFFECTstd RMSE {error}"
    );
    ...
    .expect("negative signed asymTIPREDEFFECTstd");
    assert!((negative + expected).abs() < 1e-15);
}

#[test]
fn standardised_asymptotic_time_independent_effect_refuses_non_event_clocks_and_does_not_keep_zero_q_or_v()
 {
    assert_eq!(
        recover_standardised_asymptotic_time_independent_predictor_effect(
            0.2,
