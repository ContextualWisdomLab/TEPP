...
#[test]
fn standardised_initial_trait_variance_recovers_driver_2017_era_correlation() {
    let effect = 0.5_f64;
    let trait_variance = 1.6_f64;
    let recovered =
    ...
    assert!(
        recovered_error < 1e-15,
        "Driver et al. (2017, 2017-era T0TRAITVARstd): RMSE {recovered_error} for extra / extra = 1"
    );
    ...
    let unstandardised_error = (extra - 1.0).abs();
    assert!(
        recovered_error < unstandardised_error,
        "Driver et al. (2017, 2017-era T0TRAITVAR): unstandardised extra RMSE {unstandardised_error} must exceed T0TRAITVARstd RMSE {recovered_error}"
    );
    ...
    let extra_error = (added_t0 - 1.0).abs();
    assert!(
        recovered_error < extra_error,
        "Driver et al. (2017, 2017-era addedT0TIPREDVAR): extra RMSE {extra_error} must exceed T0TRAITVARstd RMSE {recovered_error}"
    );
    assert_eq!(
        refuse_unstandardised_initial_trait_variance_as_standardised_initial_trait_variance(
            extra, recovered
...
#[test]
fn standardised_initial_trait_variance_refuses_non_event_clocks_and_does_not_keep_zero_extra() {
    assert_eq!(
        recover_standardised_initial_trait_variance(0.5, 1.6, LagClock::AssertionTime),
        Err(PsychometricError::EventTimeRequired)
    );
    assert_eq!(
        recover_standardised_initial_trait_variance(0.5, 1.6, LagClock::KnowledgeCutoff),
        Err(PsychometricError::EventTimeRequired)
    );
    assert_eq!(
...
#[test]
fn initial_trait_variance_std_clock_path_is_runtime_opaque() {
    let clocks = [
        LagClock::SystemTime,
        LagClock::DocumentTime,
        LagClock::AssertionTime,
    ...
    let non_event = clocks[non_event_index];
    assert_eq!(
        recover_standardised_initial_trait_variance(0.5, 1.6, non_event),
        Err(PsychometricError::EventTimeRequired)
    );
}
