...
#[test]
fn standardised_time_independent_predictor_mean_recovers_driver_page_sixteen_after_positive_tipredvar()
 {
    let mean = 0.8_f64;
    ...
    assert!(
        error < 1e-15,
        "Driver et al. (2017, p. 16 TIPREDMEANSstd): RMSE {error} for μ_z / √v"
    );
    let unstd_rmse = (mean - expected).abs();
    assert!(
        error < unstd_rmse,
        "Driver et al. (2017, p. 16): unstandardised TIPREDMEANS RMSE {unstd_rmse} must exceed TIPREDMEANSstd RMSE {error}"
    );
    let within =
    ...
    assert!(
        error < within_rmse,
        "Driver et al. (2017, p. 16): μ_z / √asymDIFFUSION RMSE {within_rmse} must exceed TIPREDMEANSstd RMSE {error}"
    );
    let larger =
    ...
    assert!(
        larger.abs() < recovered.abs(),
        "Driver et al. (2017, footnote 4): larger predictor SD shrinks TIPREDMEANSstd"
    );
}

#[test]
fn standardised_time_independent_predictor_mean_refuses_non_event_clocks_and_does_not_keep_zero_variance()
 {
    assert_eq!(
    ...
    assert_eq!(
        recover_standardised_time_independent_predictor_mean(0.8, 1.6, LagClock::KnowledgeCutoff),
        Err(PsychometricError::EventTimeRequired)
    );
    assert_eq!(
    ...
    let zero = recover_standardised_time_independent_predictor_mean(0.0, 1.6, LagClock::EventTime)
        .expect("zero mean");
    assert_eq!(zero.to_bits(), 0.0_f64.to_bits());
}
