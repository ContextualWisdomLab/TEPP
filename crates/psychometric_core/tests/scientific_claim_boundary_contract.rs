...
#[test]
fn standardised_time_dependent_predictor_variance_is_not_unstandardised_tipredstd_or_manifeststd() {
    ...
    assert!(
        (recovered - 1.0).abs() < 1e-15,
        "Driver et al. (2017, p. 16 / 2017-era summary.ctsemFit.R): TDPREDVARstd is v/v = 1"
    );
    let larger_variance =
    ...
    assert!(
        ...
        "Driver et al. (2017, p. 16): TIPREDVARstd and TDPREDVARstd equal 1 and remain distinct named quantities"
    );
    let manifest_std =
        recover_standardised_manifest_variance(0.4, LagClock::EventTime).expect("MANIFESTVARstd");
    assert!(
        (manifest_std - recovered).abs() < 1e-15,
        "Driver et al. (2017, p. 16): MANIFESTVARstd and TDPREDVARstd equal 1 and remain distinct named quantities"
    );
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedTimeDependentPredictorVarianceRequiresPositivePredictorVariance
        )
    );
    assert_eq!(
        recover_standardised_time_dependent_predictor_variance(
            predictor_variance,
            LagClock::DocumentTime
        ),
        Err(psychometric_core::PsychometricError::EventTimeRequired)
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::UnstandardisedTimeDependentPredictorVarianceIsNotStandardisedTimeDependentPredictorVariance
        )
    );
    assert_eq!(
        refuse_standardised_time_independent_predictor_variance_as_standardised_time_dependent_predictor_variance(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedTimeIndependentPredictorVarianceIsNotStandardisedTimeDependentPredictorVariance
        )
    );
    assert_eq!(
        refuse_standardised_manifest_variance_as_standardised_time_dependent_predictor_variance(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedManifestVarianceIsNotStandardisedTimeDependentPredictorVariance
        )
    );
}
