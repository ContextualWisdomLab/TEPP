...
#[test]
#[allow(clippy::too_many_lines)]
fn standardised_time_dependent_effect_is_not_unstandardised_or_tipred_or_discrete_or_trait() {
    ...
    assert!(
        ...
        "Driver et al. (2017, Table 2): unstandardised TDPREDEFFECT is not TDPREDEFFECTstd"
    );
    let time_independent = coefficient * predictor_variance.sqrt() / stationary.sqrt();
    assert!(
        (time_independent - recovered).abs() < 1e-15,
        "equal numbers when M = B remain distinct named quantities"
    );
    let discrete = recover_discrete_time_independent_predictor_effect(
        coefficient,
        1.0,
    ...
    assert!(
        (discrete - recovered).abs() > 1e-3,
        "Driver et al. (2017, footnote 4): A^{{-1}}[e^{{A Δt}} − I] M · √v / √p is not TDPREDEFFECTstd"
    );
    let added = recover_asymptotic_time_independent_predictor_variance(
        coefficient,
        predictor_variance,
    ...
    assert!(
        ...
        "Driver et al. (2017, footnote 4): m · √v / √(trait + p + added) is not TDPREDEFFECTstd"
    );
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::UnstandardisedTimeDependentEffectIsNotStandardisedTimeDependentEffect
        )
    );
    assert_eq!(
        refuse_standardised_time_independent_effect_as_standardised_time_dependent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedTimeIndependentEffectIsNotStandardisedTimeDependentEffect
        )
    );
    assert_eq!(
        refuse_discrete_standardised_time_dependent_effect_as_standardised_time_dependent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::DiscreteStandardisedTimeDependentEffectIsNotStandardisedTimeDependentEffect
        )
    );
    assert_eq!(
        refuse_trait_contaminated_time_dependent_effect_as_standardised_time_dependent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::TraitContaminatedTimeDependentEffectIsNotStandardisedTimeDependentEffect
        )
    );
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedTimeDependentEffectRequiresPositiveWithinSubjectVariance
        )
    );
    assert_eq!(
        recover_standardised_time_dependent_predictor_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedTimeDependentEffectRequiresPositivePredictorVariance
        )
    );
    assert_eq!(
        recover_standardised_time_dependent_predictor_effect(
