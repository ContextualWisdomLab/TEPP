...
fn initial_time_independent_predictor_variance_is_not_asymptotic_std_t0var_or_trait() {
    ...
    assert!(
        ...
        "Driver et al. (2017, 2017-era addedT0TIPREDVAR): t0_b² v RMSE"
    );
    ...
    assert!(
        ...
        "Driver et al. (2017, §7.2): addedT0TIPREDVAR is not addedTIPREDVAR"
    );
    assert!(
        (recovered - standardised).abs() > 1e-3,
        "Driver et al. (2017, Table 3): addedT0TIPREDVAR is not T0TIPREDEFFECTstd"
    );
    assert!(
        (recovered - initial_latent).abs() > 1e-3,
        "Driver et al. (2017, Table 2): addedT0TIPREDVAR is not free T0VAR"
    );
    assert!(
        (recovered - trait_variance).abs() > 1e-3,
        "Driver et al. (2017, §4.3): addedT0TIPREDVAR is not TRAITVAR"
    );
    assert_eq!(
        refuse_initial_time_independent_variance_as_asymptotic_time_independent_variance(
            recovered,
