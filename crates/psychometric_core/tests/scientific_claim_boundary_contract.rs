...
use psychometric_core::{
    ...
    recover_standardised_initial_latent_variance, recover_standardised_initial_trait_variance,
...
#[test]
fn standardised_initial_trait_variance_is_not_unstandardised_traitstd_or_t0varstd() {
    let effect = 0.5_f64;
    let trait_variance = 1.6_f64;
    let recovered =
        recover_standardised_initial_trait_variance(effect, trait_variance, LagClock::EventTime)
            .expect("T0TRAITVARstd");
    assert!(
        (recovered - 1.0).abs() < 1e-15,
        "Driver et al. (2017, 2017-era summary.ctsemFit.R 334–337): T0TRAITVARstd is extra/extra = 1"
    );
    let larger_extra = recover_standardised_initial_trait_variance(0.8, 6.4, LagClock::EventTime)
        .expect("T0TRAITVARstd larger extra");
    assert!(
        (larger_extra - recovered).abs() < 1e-15,
        "Driver et al. (2017, 2017-era T0TRAITVARstd): distinct positive extras recover the same 1"
    );
    let extra = effect * effect * trait_variance;
    assert!(
        (extra - recovered).abs() > 1e-3,
        "Driver et al. (2017, 2017-era T0TRAITVAR): unstandardised extra is not T0TRAITVARstd"
    );
    let trait_std = recover_standardised_trait_variance(trait_variance, LagClock::EventTime)
        .expect("TRAITVARstd");
    assert!(
        (trait_std - recovered).abs() < 1e-15,
        "Driver et al. (2017, p. 16): TRAITVARstd and T0TRAITVARstd equal 1 and remain distinct named quantities"
    );
    let t0var_std =
        recover_standardised_initial_latent_variance(trait_variance, LagClock::EventTime)
            .expect("T0VARstd");
    assert!(
        (t0var_std - recovered).abs() < 1e-15,
        "Driver et al. (2017, p. 16): T0VARstd and T0TRAITVARstd equal 1 and remain distinct named quantities"
    );
    let added_t0 = 0.3_f64 * 0.3_f64 * 4.0_f64;
    assert!(
        (added_t0 - recovered).abs() > 1e-3,
        "Driver et al. (2017, 2017-era addedT0TIPREDVAR): t0_b² v is not T0TRAITVARstd"
    );
    assert_eq!(
        recover_standardised_initial_trait_variance(0.5, 0.0, LagClock::EventTime),
        Err(
            psychometric_core::PsychometricError::StandardisedInitialTraitVarianceRequiresPositiveInitialTraitVariance
        )
    );
    assert_eq!(
        recover_standardised_initial_trait_variance(effect, trait_variance, LagClock::SystemTime),
        Err(psychometric_core::PsychometricError::EventTimeRequired)
    );
    assert_eq!(
        refuse_unstandardised_initial_trait_variance_as_standardised_initial_trait_variance(
        ...
        Err(
            psychometric_core::PsychometricError::UnstandardisedInitialTraitVarianceIsNotStandardisedInitialTraitVariance
        )
    );
    assert_eq!(
        refuse_standardised_trait_variance_as_standardised_initial_trait_variance(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedTraitVarianceIsNotStandardisedInitialTraitVariance
        )
    );
    assert_eq!(
        refuse_standardised_initial_latent_variance_as_standardised_initial_trait_variance(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedInitialLatentVarianceIsNotStandardisedInitialTraitVariance
        )
    );
    assert_eq!(
        refuse_initial_time_independent_variance_as_standardised_initial_trait_variance(
        ...
        Err(
            psychometric_core::PsychometricError::InitialTimeIndependentVarianceIsNotStandardisedInitialTraitVariance
        )
    );
}
