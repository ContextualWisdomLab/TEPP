...
use psychometric_core::{
    ...
    recover_standardised_discrete_drift, recover_standardised_initial_latent_mean,
    recover_standardised_initial_latent_variance,
...
fn predetermined_later_latent_variance_is_not_stationary_discrete_or_initial() {
    ...
    assert!(
        (recovered - stationary_later).abs() > 1e-3,
        "Driver et al. (2017, §4.3 predetermined T0VAR): free p_0 is not −q/(2a)"
    );
    assert!(
        (recovered - free_discrete).abs() > 1e-3,
        "Driver et al. (2017, §4.3 predetermined T0VAR): trait and addedTIPREDVAR do not enter Q_Δt"
    );
    assert!(
        (recovered - initial_latent_variance).abs() > 1e-3,
        "Driver et al. (2017, §4.3 predetermined T0VAR): e^{{2aΔt}}p_0+Q_Δt is not p_0"
    );
    assert_eq!(
        refuse_predetermined_later_latent_variance_as_stationary_later_latent_variance(
            recovered,
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaterLatentVarianceIsNotStationaryLaterLatentVariance
        )
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaterLatentVarianceIsNotInitialLatentVariance
        )
    );
}
...
fn predetermined_later_observed_variance_is_not_manifest_latent_or_stationary() {
    ...
    assert!(
        (recovered - measurement_error).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 of predetermined later §4.3 T0VAR): Var(y_t) is not MANIFESTVAR"
    );
    assert!(
        (recovered - latent).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 of predetermined later §4.3 T0VAR): Var(y_t) is not later T0VAR"
    );
    assert!(
        (recovered - stationary_later).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 of predetermined later §4.3 T0VAR): free p_0 is not −q/(2a)"
    );
    assert_eq!(
...
fn predetermined_lagged_latent_covariance_is_not_stationary_later_or_decayed() {
    ...
    .expect("addedTIPREDVAR");
    let expected =
        trait_variance + initial_latent_variance * (log_rate * event_delta).exp() + added;
    ...
    assert!(
        ...
        "Driver et al. (2017, §4.3 predetermined T0VAR): lagged omits Q_Δt"
    );
    assert!(
        (recovered - decayed_total).abs() > 1e-3,
        "Driver et al. (2017, §4.3 predetermined T0VAR): trait and addedTIPREDVAR do not decay"
    );
    assert!(
        (recovered - initial_latent_variance).abs() > 1e-3,
        "Driver et al. (2017, §4.3 predetermined T0VAR): e^{{aΔt}}p_0 is not p_0"
    );
    assert_eq!(
        refuse_predetermined_lagged_latent_covariance_as_stationary_lagged_covariance(
            recovered,
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaggedLatentCovarianceIsNotStationaryLaggedCovariance
        )
    );
    assert_eq!(
        refuse_predetermined_lagged_latent_covariance_as_later_latent_variance(recovered, later),
        Err(
            psychometric_core::PsychometricError::PredeterminedLaggedLatentCovarianceIsNotLaterLatentVariance
        )
    );
    assert_eq!(
        refuse_predetermined_lagged_latent_covariance_as_decayed_total(recovered, decayed_total),
        Err(
            psychometric_core::PsychometricError::PredeterminedLaggedLatentCovarianceIsNotDecayedTotal
        )
    );
    assert_eq!(
        refuse_predetermined_lagged_latent_covariance_as_initial_latent_variance(
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaggedLatentCovarianceIsNotInitialLatentVariance
        )
    );
}
...
fn predetermined_lagged_observed_covariance_is_not_manifest_later_or_stationary() {
    ...
    assert!(
        (recovered - measurement_error).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 of predetermined lagged §4.3 T0VAR): cov(y) is not MANIFESTVAR"
    );
    assert!(
        (recovered - latent).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 of predetermined lagged §4.3 T0VAR): cov(y) is not lagged T0VAR"
    );
    assert!(
        (recovered - later).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 of predetermined lagged §4.3 T0VAR): lagged omits Q_Δt and θ"
    );
    assert!(
        (recovered - stationary_lagged).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 of predetermined lagged §4.3 T0VAR): free p_0 is not −q/(2a)"
    );
    assert_eq!(
        refuse_predetermined_lagged_latent_covariance_as_observed_covariance(latent, recovered),
        Err(
            psychometric_core::PsychometricError::PredeterminedLaggedLatentCovarianceIsNotObservedCovariance
        )
    );
    assert_eq!(
        refuse_measurement_error_as_predetermined_lagged_observed_covariance(
        ...
        Err(
            psychometric_core::PsychometricError::MeasurementErrorIsNotPredeterminedLaggedObservedCovariance
        )
    );
    assert_eq!(
        refuse_predetermined_later_observed_variance_as_predetermined_lagged_observed_covariance(
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaterObservedVarianceIsNotPredeterminedLaggedObservedCovariance
        )
    );
    assert_eq!(
        refuse_stationary_lagged_observed_covariance_as_predetermined_lagged_observed_covariance(
...
fn predetermined_later_start_later_latent_variance_is_not_later_lagged_or_stationary() {
    ...
    assert!(
        (recovered - later).abs() > 1e-3,
        "Driver et al. (2017, §4.3 startoffset): later-start later-occasion adds Q_s"
    );
    assert!(
        (recovered - later_lagged).abs() > 1e-3,
        "Driver et al. (2017, §4.3 startoffset): later-occasion variance is not lagged covariance"
    );
    assert!(
        (recovered - stationary_later).abs() > 1e-3,
        "Driver et al. (2017, §4.3 predetermined later-start later): free p_0 is not −q/(2a)"
    );
    assert!(
        (recovered - decayed_later).abs() > 1e-3,
        "Driver et al. (2017, §4.3 startoffset): trait and addedTIPREDVAR do not enter Q_s"
    );
    assert!(
        (recovered - lag_interval).abs() > 1e-3,
        "Driver et al. (2017, §4.3 startoffset): ignoring startoffset omits e^{{2as}} Q_u"
    );
    assert_eq!(
        refuse_predetermined_later_start_later_latent_variance_as_later_latent_variance(
            recovered, later
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaterStartLaterLatentVarianceIsNotLaterLatentVariance
        )
    );
    assert_eq!(
        refuse_predetermined_later_start_later_latent_variance_as_later_lagged_covariance(
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaterStartLaterLatentVarianceIsNotLaterLaggedCovariance
        )
    );
    assert_eq!(
        refuse_predetermined_later_start_later_latent_variance_as_stationary_later_latent_variance(
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaterStartLaterLatentVarianceIsNotStationaryLaterLatentVariance
        )
    );
    assert_eq!(
        refuse_predetermined_later_start_later_latent_variance_as_decayed_later_total(
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaterStartLaterLatentVarianceIsNotDecayedLaterTotal
        )
    );
    assert_eq!(
        refuse_predetermined_later_start_later_latent_variance_as_lag_interval_later_latent_variance(
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaterStartLaterLatentVarianceIsNotLagIntervalLaterLatentVariance
        )
    );
}
...
#[test]
#[allow(clippy::too_many_lines)]
fn predetermined_later_start_later_observed_variance_is_not_manifest_later_or_stationary() {
    let trait_variance = 1.0_f64;
    let initial_latent_variance = 2.0_f64;
    let diffusion = 0.4_f64;
    let log_rate = -0.134_488_942_f64;
    ...
    assert!(
        (recovered - measurement_error).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 of later-start later-occasion predetermined T0VAR): Var(y) is not MANIFESTVAR"
    );
    assert!(
        (recovered - latent).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 of later-start later-occasion predetermined T0VAR): Var(y) is not later-start later T0VAR"
    );
    assert!(
        (recovered - later).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 of later-start later-occasion predetermined T0VAR): later at u omits Q_s"
    );
    assert!(
        (recovered - later_lagged).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 of later-start later-occasion predetermined T0VAR): lagged omits Q_s and θ"
    );
    assert!(
        (recovered - stationary).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 of later-start later-occasion predetermined T0VAR): free p_0 is not −q/(2a)"
    );
    assert_eq!(
        refuse_predetermined_later_start_later_latent_variance_as_observed_variance(
            latent, recovered
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaterStartLaterLatentVarianceIsNotObservedVariance
        )
    );
    assert_eq!(
        refuse_measurement_error_as_predetermined_later_start_later_observed_variance(
        ...
        Err(
            psychometric_core::PsychometricError::MeasurementErrorIsNotPredeterminedLaterStartLaterObservedVariance
        )
    );
    assert_eq!(
        refuse_predetermined_later_observed_variance_as_predetermined_later_start_later_observed_variance(
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaterObservedVarianceIsNotPredeterminedLaterStartLaterObservedVariance
        )
    );
    assert_eq!(
        refuse_predetermined_later_lagged_observed_covariance_as_predetermined_later_start_later_observed_variance(
        ...
        Err(
            psychometric_core::PsychometricError::PredeterminedLaterLaggedObservedCovarianceIsNotPredeterminedLaterStartLaterObservedVariance
        )
    );
    assert_eq!(
        refuse_stationary_later_observed_variance_as_predetermined_later_start_later_observed_variance(
...
#[test]
fn standardised_continuous_diffusion_is_not_unstandardised_or_trait_contaminated() {
    let diffusion = 0.4_f64;
    let log_rate = -0.5_f64;
    let recovered =
    ...
    let within = recover_stationary_latent_variance(diffusion, log_rate, LagClock::EventTime)
        .expect("asymDIFFUSION");
    assert!(
        (recovered - (diffusion / within)).abs() < 1e-15,
        "Driver et al. (2017, p. 16 / footnote 4): scalar DIFFUSIONstd is q / asymDIFFUSION"
    );
    ...
    assert!(
        ...
        "Driver et al. (2017, Eq. 4): Q_Δt / p is not −2 a"
    );
    let trait_variance = 1.0_f64;
    let total =
        recover_trait_plus_state_latent_variance(trait_variance, within).expect("trait+state var");
    ...
    assert!(
        (contaminated - recovered).abs() > 1e-3,
        "Driver et al. (2017, footnote 4 / §7.1): TRAITVAR contaminates the continuous-diffusion ratio"
    );
    assert_eq!(
        recover_standardised_continuous_diffusion(0.0, log_rate, LagClock::EventTime),
        Err(
            psychometric_core::PsychometricError::StandardisedContinuousDiffusionRequiresPositiveWithinSubjectVariance
        )
    );
    assert_eq!(
        recover_standardised_continuous_diffusion(diffusion, 0.5, LagClock::EventTime),
        Err(psychometric_core::PsychometricError::StationaryVarianceRequiresStableDrift)
    );
    assert_eq!(
        refuse_unstandardised_continuous_diffusion_as_standardised_continuous_diffusion(
        ...
        Err(
            psychometric_core::PsychometricError::UnstandardisedContinuousDiffusionIsNotStandardisedContinuousDiffusion
        )
    );
    assert_eq!(
        refuse_standardised_discrete_diffusion_as_standardised_continuous_diffusion(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedDiscreteDiffusionIsNotStandardisedContinuousDiffusion
        )
    );
    assert_eq!(
        refuse_trait_contaminated_continuous_diffusion_as_standardised_continuous_diffusion(
        ...
        Err(
            psychometric_core::PsychometricError::TraitContaminatedContinuousDiffusionIsNotStandardisedContinuousDiffusion
        )
    );
    assert_eq!(
        refuse_trait_variance_as_standardisation_variance(trait_variance, within),
...
#[test]
fn standardised_continuous_drift_is_not_unstandardised_or_trait_contaminated() {
    let diffusion = 0.4_f64;
    let log_rate = -0.5_f64;
    ...
    let within = recover_stationary_latent_variance(diffusion, log_rate, LagClock::EventTime)
        .expect("asymDIFFUSION");
    assert!(
        (recovered - log_rate).abs() < 1e-15,
        "Driver et al. (2017, p. 16 / footnote 4): scalar DRIFTstd equals a after positive asymDIFFUSION"
    );
    ...
    assert!(
        (discrete - recovered).abs() > 1e-3,
        "Driver et al. (2017, Eq. 3): exp(a Δt) is not a"
    );
    let trait_variance = 1.0_f64;
    let total =
        recover_trait_plus_state_latent_variance(trait_variance, within).expect("trait+state var");
    ...
    assert!(
        (contaminated - recovered).abs() > 1e-3,
        "Driver et al. (2017, footnote 4 / §7.1): TRAITVAR contaminates the continuous auto-effect"
    );
    assert_eq!(
        recover_standardised_continuous_drift(0.0, log_rate, LagClock::EventTime),
        Err(
            psychometric_core::PsychometricError::StandardisedContinuousDriftRequiresPositiveWithinSubjectVariance
        )
    );
    assert_eq!(
        recover_standardised_continuous_drift(diffusion, 0.5, LagClock::EventTime),
        Err(psychometric_core::PsychometricError::StationaryVarianceRequiresStableDrift)
    );
    assert_eq!(
        refuse_unstandardised_continuous_drift_as_standardised_continuous_drift(
        ...
        Err(
            psychometric_core::PsychometricError::UnstandardisedContinuousDriftIsNotStandardisedContinuousDrift
        )
    );
    assert_eq!(
        refuse_standardised_discrete_drift_as_standardised_continuous_drift(discrete, recovered),
        Err(
            psychometric_core::PsychometricError::StandardisedDiscreteDriftIsNotStandardisedContinuousDrift
        )
    );
    assert_eq!(
        refuse_trait_contaminated_continuous_drift_as_standardised_continuous_drift(
        ...
        Err(
            psychometric_core::PsychometricError::TraitContaminatedContinuousDriftIsNotStandardisedContinuousDrift
        )
    );
    assert_eq!(
        refuse_trait_variance_as_standardisation_variance(trait_variance, within),
...
#[allow(clippy::too_many_lines)]
#[test]
fn standardised_asymptotic_time_independent_effect_is_not_unstandardised_or_trait_contaminated() {
    let diffusion = 0.4_f64;
    let log_rate = -0.5_f64;
    ...
    .expect("asymTIPREDEFFECT");
    assert!(
        (recovered - unit * predictor_variance.sqrt() / within.sqrt()).abs() < 1e-15,
        "Driver et al. (2017, p. 16 / footnote 4): asymTIPREDEFFECTstd is (−B/a)·√v/√p"
    );
    ...
    assert!(
        (discrete_std - recovered).abs() > 1e-3,
        "Driver et al. (2017, Eq. 3): finite-interval standardised TIPRED is not asymTIPREDEFFECTstd"
    );
    let trait_variance = 1.0_f64;
    let total =
        recover_trait_plus_state_latent_variance(trait_variance, within).expect("trait+state var");
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedAsymptoticTimeIndependentEffectRequiresPositiveWithinSubjectVariance
        )
    );
    assert_eq!(
        recover_standardised_asymptotic_time_independent_predictor_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedAsymptoticTimeIndependentEffectRequiresPositivePredictorVariance
        )
    );
    assert_eq!(
        recover_standardised_asymptotic_time_independent_predictor_effect(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::UnstandardisedAsymptoticTimeIndependentEffectIsNotStandardisedAsymptoticTimeIndependentEffect
        )
    );
    assert_eq!(
        refuse_standardised_discrete_time_independent_effect_as_standardised_asymptotic_time_independent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedDiscreteTimeIndependentEffectIsNotStandardisedAsymptoticTimeIndependentEffect
        )
    );
    assert_eq!(
        refuse_trait_contaminated_asymptotic_time_independent_effect_as_standardised_asymptotic_time_independent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::TraitContaminatedAsymptoticTimeIndependentEffectIsNotStandardisedAsymptoticTimeIndependentEffect
        )
    );
    assert_eq!(
        refuse_trait_variance_as_standardisation_variance(trait_variance, within),
...
#[allow(clippy::too_many_lines)]
#[test]
fn standardised_discrete_time_independent_effect_is_not_unstandardised_or_trait_contaminated() {
fn standardised_continuous_time_independent_effect_is_not_unstandardised_or_trait_contaminated() {
    let diffusion = 0.4_f64;
    let log_rate = -0.5_f64;
    let coefficient = 0.3_f64;
    let predictor_variance = 1.0_f64;
    ...
    assert!(
        (recovered - unit * predictor_variance.sqrt() / within.sqrt()).abs() < 1e-15,
        "Driver et al. (2017, p. 16 / footnote 4): finite-interval TIPREDEFFECTstd is A^{{-1}}[e^{{AΔt}}−I]B·√v/√p"
    );
    ...
    let within = recover_stationary_latent_variance(diffusion, log_rate, LagClock::EventTime)
        .expect("asymDIFFUSION");
    assert!(
        (recovered - coefficient * predictor_variance.sqrt() / within.sqrt()).abs() < 1e-15,
        "Driver et al. (2017, p. 16 / footnote 4): TIPREDEFFECTstd is B·√v/√p"
    );
    ...
    assert!(
        ...
        "Driver et al. (2017, Eq. 3 / p. 16): asymTIPREDEFFECTstd is not the finite-interval map"
        LagClock::EventTime,
    )
    .expect("asymTIPREDEFFECTstd");
    assert!(
        (asymptotic - recovered).abs() > 1e-3,
        "Driver et al. (2017, p. 16 / §7.2): asymTIPREDEFFECTstd is not TIPREDEFFECTstd"
    );
    let discrete_increment = recover_discrete_time_independent_predictor_effect(
        coefficient,
        1.0,
    ...
    assert!(
        (discrete_std - recovered).abs() > 1e-3,
        "Driver et al. (2017, Eq. 3): finite-interval standardised TIPRED is not TIPREDEFFECTstd"
    );
    let trait_variance = 1.0_f64;
    let total =
        recover_trait_plus_state_latent_variance(trait_variance, within).expect("trait+state var");
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedDiscreteTimeIndependentEffectRequiresPositiveWithinSubjectVariance
        )
    );
    assert_eq!(
        recover_standardised_discrete_time_independent_predictor_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedContinuousTimeIndependentEffectRequiresPositiveWithinSubjectVariance
        )
    );
    assert_eq!(
        recover_standardised_continuous_time_independent_predictor_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedDiscreteTimeIndependentEffectRequiresPositivePredictorVariance
        )
    );
    assert_eq!(
        recover_standardised_discrete_time_independent_predictor_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedContinuousTimeIndependentEffectRequiresPositivePredictorVariance
        )
    );
    assert_eq!(
        recover_standardised_continuous_time_independent_predictor_effect(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::UnstandardisedDiscreteTimeIndependentEffectIsNotStandardisedDiscreteTimeIndependentEffect
        )
    );
    assert_eq!(
        refuse_standardised_asymptotic_time_independent_effect_as_standardised_discrete_time_independent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedAsymptoticTimeIndependentEffectIsNotStandardisedDiscreteTimeIndependentEffect
        )
    );
    assert_eq!(
        refuse_trait_contaminated_discrete_time_independent_effect_as_standardised_discrete_time_independent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::UnstandardisedContinuousTimeIndependentEffectIsNotStandardisedContinuousTimeIndependentEffect
        )
    );
    assert_eq!(
        refuse_standardised_asymptotic_time_independent_effect_as_standardised_continuous_time_independent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedAsymptoticTimeIndependentEffectIsNotStandardisedContinuousTimeIndependentEffect
        )
    );
    assert_eq!(
        refuse_standardised_discrete_time_independent_effect_as_standardised_continuous_time_independent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedDiscreteTimeIndependentEffectIsNotStandardisedContinuousTimeIndependentEffect
        )
    );
    assert_eq!(
        refuse_trait_contaminated_continuous_time_independent_effect_as_standardised_continuous_time_independent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::TraitContaminatedDiscreteTimeIndependentEffectIsNotStandardisedDiscreteTimeIndependentEffect
...
#[allow(clippy::too_many_lines)]
#[test]
fn standardised_continuous_intercept_is_not_unstandardised_or_trait_contaminated() {
    let diffusion = 0.4_f64;
    let log_rate = -0.5_f64;
    let intercept = 0.3_f64;
...
fn standardised_continuous_time_dependent_effect_is_not_unstandardised_or_trait_contaminated() {
    ...
    let within = recover_stationary_latent_variance(diffusion, log_rate, LagClock::EventTime)
        .expect("asymDIFFUSION");
    assert!(
        (recovered - intercept / within.sqrt()).abs() < 1e-15,
    ...
    assert!(
        (asymptotic_std - recovered).abs() > 1e-3,
        "Driver et al. (2017, Table 2): asymCINTstd is not CINTstd"
    );
    ...
    assert!(
        (discrete_std - recovered).abs() > 1e-3,
        "Driver et al. (2017, Eq. 3): finite-interval standardised CINT is not CINTstd"
    .expect("TDPREDEFFECTstd");
    let within = recover_stationary_latent_variance(diffusion, log_rate, LagClock::EventTime)
        .expect("asymDIFFUSION");
    assert!(
        (recovered - coefficient * predictor_variance.sqrt() / within.sqrt()).abs() < 1e-15,
        "Driver et al. (2017, p. 16 / footnote 4): TDPREDEFFECTstd is m·√v/√p"
    );
    ...
    assert_eq!(
        tipred.to_bits(),
        recovered.to_bits(),
        "Driver et al. (2017, Table 2): equal numbers when M=B are still distinct named quantities"
    );
    let discrete_increment = recover_discrete_time_independent_predictor_effect(
        coefficient,
        1.0,
    ...
    assert!(
        (discrete_std - recovered).abs() > 1e-3,
        "Driver et al. (2017, Eq. 3): intercept-style standardised TDPRED is not TDPREDEFFECTstd"
    );
    let trait_variance = 1.0_f64;
    let total =
        recover_trait_plus_state_latent_variance(trait_variance, within).expect("trait+state var");
    ...
    assert!(
        (contaminated - recovered).abs() > 1e-3,
        "Driver et al. (2017, footnote 4 / §7.1): TRAITVAR contaminates the process SD"
    );
    let zero =
        recover_standardised_continuous_intercept(0.0, diffusion, log_rate, LagClock::EventTime)
            .expect("zero intercept");
    assert_eq!(zero.to_bits(), 0.0_f64.to_bits());
    assert_eq!(
        recover_standardised_continuous_intercept(0.3, 0.0, log_rate, LagClock::EventTime),
        Err(
            psychometric_core::PsychometricError::StandardisedContinuousInterceptRequiresPositiveWithinSubjectVariance
        )
    );
    assert_eq!(
        recover_standardised_continuous_intercept(0.3, diffusion, 0.5, LagClock::EventTime),
        Err(psychometric_core::PsychometricError::StationaryVarianceRequiresStableDrift)
    );
    assert_eq!(
        refuse_unstandardised_continuous_intercept_as_standardised_continuous_intercept(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedAsymptoticContinuousInterceptIsNotStandardisedContinuousIntercept
        )
    );
    assert_eq!(
        refuse_standardised_discrete_continuous_intercept_as_standardised_continuous_intercept(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedContinuousTimeDependentEffectRequiresPositiveWithinSubjectVariance
        )
    );
    assert_eq!(
        recover_standardised_continuous_time_dependent_predictor_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedContinuousTimeDependentEffectRequiresPositivePredictorVariance
        )
    );
    assert_eq!(
        recover_standardised_continuous_time_dependent_predictor_effect(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::UnstandardisedContinuousTimeDependentEffectIsNotStandardisedContinuousTimeDependentEffect
        )
    );
    assert_eq!(
        refuse_standardised_continuous_time_independent_effect_as_standardised_continuous_time_dependent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedContinuousTimeIndependentEffectIsNotStandardisedContinuousTimeDependentEffect
        )
    );
    assert_eq!(
        refuse_standardised_discrete_time_dependent_effect_as_standardised_continuous_time_dependent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedDiscreteContinuousInterceptIsNotStandardisedContinuousIntercept
        )
    );
    assert_eq!(
        refuse_trait_contaminated_continuous_intercept_as_standardised_continuous_intercept(
            psychometric_core::PsychometricError::StandardisedDiscreteTimeDependentEffectIsNotStandardisedContinuousTimeDependentEffect
        )
    );
    assert_eq!(
        refuse_trait_contaminated_continuous_time_dependent_effect_as_standardised_continuous_time_dependent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::TraitContaminatedContinuousTimeDependentEffectIsNotStandardisedContinuousTimeDependentEffect
        )
    );
    assert_eq!(
        refuse_trait_variance_as_standardisation_variance(trait_variance, within),
...
#[allow(clippy::too_many_lines)]
#[test]
fn standardised_initial_time_dependent_effect_is_not_unstandardised_or_trait_contaminated() {
    ...
    assert!(
        (recovered - coefficient * predictor_variance.sqrt() / initial_variance.sqrt()).abs()
            < 1e-15,
        "Driver et al. (2017, Table 3 / footnote 4): T0TDPREDEFFECTstd is t0_m·√v/√p_0"
    );
    ...
    assert!(
        ...
        "Driver et al. (2017, p. 16 / Table 3): TDPREDEFFECTstd is not T0TDPREDEFFECTstd"
    );
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedInitialTimeDependentEffectRequiresPositiveInitialLatentVariance
        )
    );
    assert_eq!(
        recover_standardised_initial_time_dependent_predictor_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedInitialTimeDependentEffectRequiresPositivePredictorVariance
        )
    );
    assert_eq!(
        recover_standardised_initial_time_dependent_predictor_effect(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::UnstandardisedInitialTimeDependentEffectIsNotStandardisedInitialTimeDependentEffect
        )
    );
    assert_eq!(
        refuse_standardised_continuous_time_dependent_effect_as_standardised_initial_time_dependent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedContinuousTimeDependentEffectIsNotStandardisedInitialTimeDependentEffect
        )
    );
    assert_eq!(
        refuse_standardised_initial_time_independent_effect_as_standardised_initial_time_dependent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedInitialTimeIndependentEffectIsNotStandardisedInitialTimeDependentEffect
        )
    );
    assert_eq!(
        refuse_trait_contaminated_initial_time_dependent_effect_as_standardised_initial_time_dependent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::TraitContaminatedInitialTimeDependentEffectIsNotStandardisedInitialTimeDependentEffect
        )
    );
    assert_eq!(
...
#[test]
fn initial_time_dependent_predictor_variance_is_not_covariance_or_added_t0_tipred_var() {
    let coefficient = 0.3_f64;
    ...
    assert!(
        (recovered - coefficient * coefficient * predictor_variance).abs() < 1e-15,
        "Driver et al. (2017, Table 2 TDPREDVAR / analog of addedT0TIPREDVAR): extra is t0_m² v"
    );
    let doubled =
        recover_initial_time_dependent_predictor_variance(coefficient, 8.0, LagClock::EventTime)
            .expect("doubled v");
    ...
    assert!(
        ...
        "Driver et al. (2017, Table 3): T0TDPREDEFFECTstd is not the first-occasion TD extra"
    );
    let covariance = coefficient * predictor_variance;
    assert!(
        (covariance - recovered).abs() > 1e-3,
        "Driver et al. (2017, Table 2): T0TDPREDCOV is not t0_m² v"
    );
    assert_eq!(
    ...
    assert_eq!(
        recover_initial_time_dependent_predictor_variance(0.3, 0.0, LagClock::EventTime)
            .expect("zero v")
            .to_bits(),
        0.0_f64.to_bits()
    );
...
fn initial_time_dependent_variance_refusal_boundaries_stay_distinct() {
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::InitialTimeDependentVarianceIsNotInitialTimeIndependentVariance
        )
    );
    assert_eq!(
        refuse_initial_time_dependent_variance_as_standardised_initial_time_dependent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::InitialTimeDependentVarianceIsNotStandardisedInitialTimeDependentEffect
        )
    );
    assert_eq!(
        refuse_initial_time_dependent_variance_as_initial_time_dependent_covariance(
        ...
        Err(
            psychometric_core::PsychometricError::InitialTimeDependentVarianceIsNotInitialTimeDependentCovariance
        )
    );
    assert_eq!(
        refuse_initial_time_dependent_variance_as_initial_latent_variance(recovered, 1.6),
        Err(psychometric_core::PsychometricError::InitialTimeDependentVarianceIsNotInitialLatentVariance)
    );
    assert_eq!(
...
#[allow(clippy::too_many_lines)]
#[test]
fn standardised_manifest_variance_is_not_unstandardised_or_manifesttraitvarstd() {
    ...
    assert_eq!(
        recover_standardised_manifest_variance(measurement_error, LagClock::SystemTime),
        Err(psychometric_core::PsychometricError::EventTimeRequired)
    );
    assert_eq!(
        refuse_unstandardised_manifest_variance_as_standardised_manifest_variance(
...
#[allow(clippy::too_many_lines)]
#[test]
fn standardised_time_independent_predictor_variance_is_not_unstandardised_or_manifestvarstd() {
...
#[allow(clippy::too_many_lines)]
#[test]
fn standardised_asymptotic_diffusion_is_not_unstandardised_or_diffusionstd() {
    let diffusion = 0.4_f64;
    ...
    let within = recover_stationary_latent_variance(diffusion, log_rate, LagClock::EventTime)
        .expect("asymDIFFUSION");
    assert!((within - recovered).abs() > 1e-3);
    let predictor_std =
        recover_standardised_time_independent_predictor_variance(within, LagClock::EventTime)
            .expect("TIPREDVARstd");
    assert_eq!(
        predictor_std.to_bits(),
        recovered.to_bits(),
        "Driver et al. (2017, p. 16): TIPREDVARstd and asymDIFFUSIONstd equal 1 and remain distinct named quantities"
    );
    let diffusion_std =
        recover_standardised_continuous_diffusion(diffusion, log_rate, LagClock::EventTime)
            .expect("DIFFUSIONstd");
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedAsymptoticDiffusionRequiresPositiveWithinSubjectVariance
        )
    );
    assert_eq!(
    ...
    assert_eq!(
        recover_standardised_asymptotic_diffusion(diffusion, log_rate, LagClock::SystemTime),
        Err(psychometric_core::PsychometricError::EventTimeRequired)
    );
    assert_eq!(
        refuse_unstandardised_asymptotic_diffusion_as_standardised_asymptotic_diffusion(
...
#[allow(clippy::too_many_lines)]
#[test]
fn standardised_discrete_continuous_intercept_is_not_unstandardised_or_cintstd() {
    let intercept = 0.3_f64;
    let diffusion = 0.4_f64;
    let log_rate = -0.25_f64;
    let event_delta = 1.0_f64;
    ...
    assert!(
        ...
        "Driver et al. (2017, p. 16 / footnote 4): discreteCINTstd is discreteCINT / √p"
    );
    let continuous_std = intercept / within.sqrt();
    assert!(
        (continuous_std - recovered).abs() > 1e-3,
        "Driver et al. (2017, p. 16): κ / √p does not depend on Δt and is not discreteCINTstd"
    );
    let asymptotic =
        recover_asymptotic_continuous_intercept(intercept, log_rate, LagClock::EventTime)
            .expect("asymCINT");
    ...
    assert!(
        (asymptotic_std - recovered).abs() > 1e-3,
        "Driver et al. (2017, Table 2): (-κ / a) / √p is not discreteCINTstd"
    );
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedDiscreteContinuousInterceptRequiresPositiveWithinSubjectVariance
        )
    );
    assert_eq!(
        recover_standardised_discrete_continuous_intercept(
...
fn standardised_asymptotic_continuous_intercept_is_not_unstandardised_or_cintstd() {
    ...
    let within = recover_stationary_latent_variance(diffusion, log_rate, LagClock::EventTime)
        .expect("asymDIFFUSION");
    assert!((within - 0.8).abs() < 1e-15);
    let expected = asymptotic / within.sqrt();
    assert!(
        (recovered - expected).abs() < 1e-15,
        "Driver et al. (2017, p. 16 / footnote 4): asymCINTstd is asymCINT / √p"
    );
    let continuous_std = intercept / within.sqrt();
    assert!(
        (continuous_std - recovered).abs() > 1e-3,
        "Driver et al. (2017, p. 16): κ / √p is not asymCINTstd"
    );
    ...
    assert!(
        (discrete_std - recovered).abs() > 1e-3,
        "Driver et al. (2017, p. 16): discreteCINTstd depends on Δt and is not asymCINTstd"
    );
    let later = recover_standardised_discrete_continuous_intercept(
        intercept,
        diffusion,
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedAsymptoticContinuousInterceptRequiresPositiveWithinSubjectVariance
        )
    );
    assert_eq!(
        recover_standardised_asymptotic_continuous_intercept(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedDiscreteContinuousInterceptIsNotStandardisedAsymptoticContinuousIntercept
        )
    );
}
...
fn standardised_initial_latent_mean_is_not_unstandardised_or_t0varstd() {
    ...
    assert!(
        (unit - variance_std).abs() < 1e-15,
        "Driver et al. (2017, p. 16): equal 1 with T0VARstd remains a distinct named quantity"
    );
    let within =
    ...
    assert_eq!(
        recover_standardised_initial_latent_mean(mean, initial_variance, LagClock::SystemTime),
        Err(psychometric_core::PsychometricError::EventTimeRequired)
    );
    assert_eq!(
        refuse_unstandardised_initial_latent_mean_as_standardised_initial_latent_mean(
...
#[allow(clippy::too_many_lines)]
#[test]
fn standardised_initial_time_independent_effect_is_not_unstandardised_or_trait_contaminated() {
    let initial_variance = 1.6_f64;
    let coefficient = 0.3_f64;
    let predictor_variance = 1.0_f64;
    ...
    assert!(
        (continuous - recovered).abs() > 1e-3,
        "Driver et al. (2017, p. 16 / Table 3): TIPREDEFFECTstd is not T0TIPREDEFFECTstd"
    );
    let asymptotic = recover_standardised_asymptotic_time_independent_predictor_effect(
        coefficient,
        predictor_variance,
    ...
    assert!(
        (asymptotic - recovered).abs() > 1e-3,
        "Driver et al. (2017, p. 16 / §7.2): asymTIPREDEFFECTstd is not T0TIPREDEFFECTstd"
    );
    let trait_variance = 1.0_f64;
    let total = recover_trait_plus_state_latent_variance(trait_variance, initial_variance)
        .expect("trait+state var");
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedInitialTimeIndependentEffectRequiresPositiveInitialLatentVariance
        )
    );
    assert_eq!(
        recover_standardised_initial_time_independent_predictor_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedInitialTimeIndependentEffectRequiresPositivePredictorVariance
        )
    );
    assert_eq!(
        recover_standardised_initial_time_independent_predictor_effect(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::UnstandardisedInitialTimeIndependentEffectIsNotStandardisedInitialTimeIndependentEffect
        )
    );
    assert_eq!(
        refuse_standardised_continuous_time_independent_effect_as_standardised_initial_time_independent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedContinuousTimeIndependentEffectIsNotStandardisedInitialTimeIndependentEffect
        )
    );
    assert_eq!(
        refuse_standardised_asymptotic_time_independent_effect_as_standardised_initial_time_independent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedAsymptoticTimeIndependentEffectIsNotStandardisedInitialTimeIndependentEffect
        )
    );
    assert_eq!(
        refuse_trait_contaminated_initial_time_independent_effect_as_standardised_initial_time_independent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::TraitContaminatedContinuousInterceptIsNotStandardisedContinuousIntercept
        )
    );
    assert_eq!(
        refuse_trait_variance_as_standardisation_variance(trait_variance, within),
...
fn standardised_initial_time_dependent_effect_is_not_unstandardised_or_trait_contaminated() {
    ...
    assert!(
        (recovered - coefficient * predictor_variance.sqrt() / initial_variance.sqrt()).abs()
            < 1e-15,
        "Driver et al. (2017, Table 3 / footnote 4): T0TDPREDEFFECTstd is t0_m·√v_x/√p_0"
    );
    let larger_p0 = recover_standardised_initial_time_dependent_predictor_effect(
        coefficient,
        predictor_variance,
    ...
    assert_eq!(
        same_numbers_ti.to_bits(),
        recovered.to_bits(),
        "Driver et al. (2017, Table 3): same numbers yield the same product"
    );
    let continuous = recover_standardised_continuous_time_independent_predictor_effect(
        coefficient,
        predictor_variance,
    ...
    assert!(
        (continuous - recovered).abs() > 1e-3,
        "Driver et al. (2017, p. 16 / Table 3): TIPREDEFFECTstd is not T0TDPREDEFFECTstd"
    );
    let trait_variance = 1.0_f64;
    let total = recover_trait_plus_state_latent_variance(trait_variance, initial_variance)
        .expect("trait+state var");
    ...
    assert_eq!(
        recover_standardised_initial_time_dependent_predictor_effect(
            ...
            predictor_variance,
            initial_variance,
fn initial_time_independent_variance_is_not_asymptotic_or_standardised_effect() {
    let coefficient = 0.3_f64;
    let predictor_variance = 4.0_f64;
    ...
    assert!(
        (recovered - coefficient * coefficient * predictor_variance).abs() < 1e-15,
        "Driver et al. (2017, Table 3 / 2017-era summary.ctsemFit.R): addedT0TIPREDVAR is t0_b² v"
    );
    ...
    assert!(
        (asymptotic - recovered).abs() > 1e-3,
        "Driver et al. (2017, §7.2 / Table 3): addedTIPREDVAR is not addedT0TIPREDVAR"
    );
    assert_eq!(
        recover_asymptotic_time_independent_predictor_variance(
            coefficient,
        ...
        Err(
            psychometric_core::PsychometricError::AsymptoticTimeIndependentEffectRequiresStableDrift
        )
    );
    ...
    assert!(
        (standardised - recovered).abs() > 1e-3,
        "Driver et al. (2017, Table 3): T0TIPREDEFFECTstd is not addedT0TIPREDVAR"
    );
    assert_eq!(
        recover_initial_time_independent_predictor_variance(
            coefficient,
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedContinuousTimeIndependentEffectIsNotStandardisedInitialTimeDependentEffect
        )
    );
    assert_eq!(
        refuse_trait_contaminated_initial_time_dependent_effect_as_standardised_initial_time_dependent_effect(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::InitialTimeIndependentVarianceIsNotAsymptoticTimeIndependentVariance
        )
    );
    assert_eq!(
        refuse_initial_time_independent_variance_as_standardised_initial_time_independent_effect(
        ...
        Err(
            psychometric_core::PsychometricError::InitialTimeIndependentVarianceIsNotStandardisedInitialTimeIndependentEffect
        )
    );
    assert_eq!(
        refuse_initial_time_independent_variance_as_initial_latent_variance(recovered, 1.6),
        Err(
            psychometric_core::PsychometricError::InitialTimeIndependentVarianceIsNotInitialLatentVariance
        )
    );
    assert_eq!(
...
#[allow(clippy::too_many_lines)]
#[test]
fn initial_time_independent_observed_variance_is_not_latent_extra_or_measurement_error() {
    let loading = 2.0_f64;
    let coefficient = 0.3_f64;
    let predictor_variance = 4.0_f64;
    ...
    assert!(
        (recovered - loading * loading * extra).abs() < 1e-15,
        "Driver et al. (2017, Eq. 5 of 2017-era addedT0TIPREDVAR): extra observed TI variance is λ² t0_b² v"
    );
    assert!(
        (extra - recovered).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5): latent extra is not observed extra"
    ...
    assert!(
        (initial_observed - recovered).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 / Table 2): λ² p_0 + θ is not extra observed TI variance"
    );
    ...
    .expect("λ² (B/a)² v");
    assert!(
        (asymptotic_observed - recovered).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 / §7.2): λ² (B/a)² v is not first-occasion extra observed TI variance"
    );
    assert_eq!(
        recover_initial_time_independent_observed_variance(
            loading,
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::InitialTimeIndependentObservedVarianceIsNotInitialTimeIndependentVariance
        )
    );
    assert_eq!(
        refuse_initial_time_independent_observed_variance_as_initial_observed_variance(
        ...
        Err(
            psychometric_core::PsychometricError::InitialTimeIndependentObservedVarianceIsNotInitialObservedVariance
        )
    );
    assert_eq!(
        refuse_initial_time_independent_observed_variance_as_asymptotic_time_independent_observed_variance(
        ...
        Err(
            psychometric_core::PsychometricError::InitialTimeIndependentObservedVarianceIsNotAsymptoticTimeIndependentObservedVariance
        )
    );
    assert_eq!(
        refuse_initial_time_independent_observed_variance_as_measurement_error(recovered, 0.1),
        Err(
            psychometric_core::PsychometricError::InitialTimeIndependentObservedVarianceIsNotMeasurementError
        )
    );
}
...
#[allow(clippy::too_many_lines)]
#[test]
fn asymptotic_time_independent_observed_variance_is_not_latent_extra_or_measurement_error() {
    let loading = 2.0_f64;
    let coefficient = 0.3_f64;
    let predictor_variance = 4.0_f64;
    let log_rate = -0.5_f64;
    ...
    assert!(
        (recovered - loading * loading * extra).abs() < 1e-15,
        "Driver et al. (2017, Eq. 5 of §7.2 addedTIPREDVAR): extra observed TI variance is λ² (B/a)² v"
    );
    assert!(
        (extra - recovered).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5): latent extra is not observed extra"
    ...
    assert!(
        (stationary_observed - recovered).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 / Table 2): λ² p + θ is not extra observed TI variance"
    );
    ...
    assert!(
        (initial_observed - recovered).abs() > 1e-3,
        "Driver et al. (2017, Eq. 5 / Table 3): λ² t0_b² v is not asymptotic extra observed TI variance"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::AsymptoticTimeIndependentObservedVarianceIsNotAsymptoticTimeIndependentVariance
        )
    );
    assert_eq!(
        refuse_asymptotic_time_independent_observed_variance_as_initial_time_independent_observed_variance(
        ...
        Err(
            psychometric_core::PsychometricError::AsymptoticTimeIndependentObservedVarianceIsNotInitialTimeIndependentObservedVariance
        )
    );
    assert_eq!(
        refuse_asymptotic_time_independent_observed_variance_as_stationary_observed_variance(
        ...
        Err(
            psychometric_core::PsychometricError::AsymptoticTimeIndependentObservedVarianceIsNotStationaryObservedVariance
        )
    );
    assert_eq!(
        refuse_asymptotic_time_independent_observed_variance_as_measurement_error(recovered, 0.1),
        Err(
            psychometric_core::PsychometricError::AsymptoticTimeIndependentObservedVarianceIsNotMeasurementError
        )
    );
}
...
#[allow(clippy::too_many_lines)]
#[test]
fn standardised_asymptotic_time_independent_variance_is_not_unstandardised_or_observed_extra() {
    let coefficient = 0.3_f64;
    let predictor_variance = 4.0_f64;
    let log_rate = -0.5_f64;
    let extra = recover_asymptotic_time_independent_predictor_variance(
    ...
    assert!(
        (recovered - 1.0).abs() < 1e-15,
        "Driver et al. (2017, 2017-era addedTIPREDVARstd): extra/extra is 1 after strictly positive extra"
    );
    assert!(
        (extra - recovered).abs() > 1e-3,
        "Driver et al. (2017, 2017-era addedTIPREDVARstd): unstandardised extra is not the correlation"
    );
    ...
    assert_eq!(
        doubled.to_bits(),
        recovered.to_bits(),
        "Driver et al. (2017, 2017-era addedTIPREDVARstd): doubling v keeps extra/extra = 1"
    );
    ...
    assert!(
        ...
        "Driver et al. (2017, Eq. 5): observed extra is not addedTIPREDVARstd"
    );
    ...
    .expect("addedT0TIPREDVAR");
    assert!(
        (initial_extra - recovered).abs() > 1e-3,
        "Driver et al. (2017, Table 3): addedT0TIPREDVAR is not addedTIPREDVARstd"
    );
    assert_eq!(
        recover_standardised_asymptotic_time_independent_predictor_variance(
            coefficient,
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedAsymptoticTimeIndependentVarianceRequiresPositiveExtraVariance
        )
    );
    assert_eq!(
        refuse_unstandardised_asymptotic_time_independent_variance_as_standardised_asymptotic_time_independent_variance(
        ...
        Err(
            psychometric_core::PsychometricError::UnstandardisedAsymptoticTimeIndependentVarianceIsNotStandardisedAsymptoticTimeIndependentVariance
        )
    );
    assert_eq!(
        refuse_asymptotic_time_independent_observed_variance_as_standardised_asymptotic_time_independent_variance(
        ...
        Err(
            psychometric_core::PsychometricError::AsymptoticTimeIndependentObservedVarianceIsNotStandardisedAsymptoticTimeIndependentVariance
        )
    );
    assert_eq!(
        refuse_initial_time_independent_variance_as_standardised_asymptotic_time_independent_variance(
        ...
        Err(
            psychometric_core::PsychometricError::InitialTimeIndependentVarianceIsNotStandardisedAsymptoticTimeIndependentVariance
        )
    );
    assert_eq!(
