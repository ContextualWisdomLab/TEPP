...
fn standardised_initial_time_dependent_effect_is_not_unstandardised_or_trait_contaminated() {
    ...
    for (grid_coefficient, grid_predictor, grid_initial) in cases {
        let grid_recovered = recover_standardised_initial_time_dependent_predictor_effect(
            grid_coefficient,
            grid_predictor,
            grid_initial,
            LagClock::EventTime,
        )
        .expect("T0TDPREDEFFECTstd grid");
        ...
        assert!(
            error.abs() < 1e-15,
            "Driver et al. (2017, Table 3): noiseless t0_m·√v/√p_0 recovers known truth"
        );
    }
    ...
    // Inline unpublished TDPREDEFFECTstd / T0TIPREDEFFECTstd scalars.
    // Those maps are not exported on this main.
    let within = -0.4_f64 / (2.0 * -0.5_f64);
    ...
    assert!(
        (initial_time_independent - recovered).abs() < 1e-15,
        "Driver et al. (2017, Table 3): equal numbers when t0_m = t0_b remain distinct named quantities"
    );
    let trait_variance = 1.0_f64;
    ...
    assert!(
        ...
        "Driver et al. (2017, footnote 4 / §7.1): t0_m·√v/√(trait+p_0+added) is not T0TDPREDEFFECTstd"
    );
    assert_eq!(
        recover_standardised_initial_time_dependent_predictor_effect(
            coefficient,
