...
#[test]
fn standardised_continuous_diffusion_is_not_unstandardised_discrete_or_total() {
    let diffusion = 0.4_f64;
    let log_rate = -0.25_f64;
    let recovered =
        recover_standardised_continuous_diffusion(diffusion, log_rate, LagClock::EventTime)
            .expect("DIFFUSIONstd");
    assert!(
        (recovered - (-2.0 * log_rate)).abs() < 1e-15,
        "Driver et al. (2017, p. 16 footnote 4): DIFFUSIONstd is q/p = −2a"
    );
    ...
    assert!(
        ...
        "Driver et al. (2017, Table 2): unstandardised DIFFUSION is not DIFFUSIONstd"
    );
    let stationary =
        recover_stationary_latent_variance(diffusion, log_rate, LagClock::EventTime).expect("p");
    let process_noise =
    ...
    assert!(
        (total_scaled - recovered).abs() > 1e-3,
        "Driver et al. (2017, footnote 4): q / (trait + p + added) is not DIFFUSIONstd"
    );
    let equal_ratio =
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::DiscreteStandardisedDiffusionIsNotStandardisedContinuousDiffusion
        )
    );
    assert_eq!(
        refuse_total_variance_scaled_diffusion_as_standardised_continuous_diffusion(
            total_scaled, recovered
        ),
        Err(
            psychometric_core::PsychometricError::TotalVarianceScaledDiffusionIsNotStandardisedContinuousDiffusion
        )
    );
    assert_eq!(
        refuse_standardised_continuous_diffusion_as_standardised_asymptotic_diffusion(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedContinuousDiffusionRequiresPositiveStationaryVariance
        )
    );
    assert_eq!(
