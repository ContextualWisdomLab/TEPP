...
#[test]
fn standardised_added_time_independent_variance_is_not_unstandardised_or_traitvarstd() {
    let effect = 0.4_f64;
    ...
    assert!(
        (recovered - 1.0).abs() < 1e-15,
        "Driver et al. (2017, p. 16 / 2017-era summary.ctsemFit.R): addedTIPREDVARstd is extra/extra = 1"
    );
    ...
    assert!(
        (trait_std - recovered).abs() < 1e-15,
        "Driver et al. (2017, p. 16): TRAITVARstd and addedTIPREDVARstd equal 1 and remain distinct named quantities"
    );
    let first_occasion_extra = 0.3_f64 * 0.3_f64 * 4.0_f64;
    assert!(
        (first_occasion_extra - recovered).abs() > 1e-3,
        "Driver et al. (2017, 2017-era addedT0TIPREDVAR): t0_b² v is not addedTIPREDVARstd"
    );
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedAsymptoticTimeIndependentVarianceRequiresPositiveAddedVariance
        )
    );
    assert_eq!(
        recover_standardised_asymptotic_time_independent_predictor_variance(
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::StandardisedTraitVarianceIsNotStandardisedAsymptoticTimeIndependentVariance
        )
    );
    assert_eq!(
        refuse_initial_time_independent_variance_as_standardised_asymptotic_time_independent_variance(
