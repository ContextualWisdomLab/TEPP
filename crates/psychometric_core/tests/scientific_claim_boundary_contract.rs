...
#[test]
fn standardised_discrete_diffusion_is_not_unstandardised_continuous_or_total() {
    let diffusion = 0.4_f64;
    let log_rate = -0.25_f64;
    let event_delta = 1.0_f64;
    ...
    assert!(
        (recovered - identity).abs() < 1e-15,
        "Driver et al. (2017, p. 16 footnote 4): discreteDIFFUSIONstd is Q_Δt / p = 1 − exp(2 a Δt)"
    );
    let later =
    ...
    assert!(
        (process_noise - recovered).abs() > 1e-3,
        "Driver et al. (2017, Eq. 3): unstandardised discreteDIFFUSION is not discreteDIFFUSIONstd"
    );
    let continuous_std = -2.0 * log_rate;
    assert!(
        (continuous_std - recovered).abs() > 1e-3,
        "Driver et al. (2017, p. 16): DIFFUSIONstd −2a is not discreteDIFFUSIONstd"
    );
    let stationary =
        recover_stationary_latent_variance(diffusion, log_rate, LagClock::EventTime).expect("p");
    ...
    assert!(
        ...
        "Driver et al. (2017, footnote 4): Q_Δt / (trait + p + added) is not discreteDIFFUSIONstd"
    );
    assert_eq!(
        refuse_unstandardised_discrete_diffusion_as_standardised_discrete_diffusion(
            process_noise,
    ...
    assert_eq!(
        ...
        Err(
            psychometric_core::PsychometricError::TotalVarianceScaledDiscreteDiffusionIsNotStandardisedDiscreteDiffusion
        )
    );
    assert_eq!(
        recover_standardised_discrete_diffusion(0.0, log_rate, event_delta, LagClock::EventTime),
        Err(
            psychometric_core::PsychometricError::StandardisedDiscreteDiffusionRequiresPositiveStationaryVariance
        )
    );
    assert_eq!(
    ...
    assert_eq!(
        recover_standardised_discrete_diffusion(diffusion, 0.25, event_delta, LagClock::EventTime),
        Err(psychometric_core::PsychometricError::StationaryVarianceRequiresStableDrift)
    );
}
