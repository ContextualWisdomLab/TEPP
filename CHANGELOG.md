...
- `tepp_api` naruon live HTTP/1.1 listener: loopback-only `NaruonLiveService` serves `POST /v1/analysis-runs` and `/v1/exports` with a 2s read/write stream deadline, loopback `Host`, RFC 3339 `knowledge_cutoff`, optional `tepp-available-time` eligibility, tenant-plus-key idempotency replay (including export over `TcpStream`), and refusal of `Transfer-Encoding`, `Proxy-Authorization`, and NIM/NVIDIA headers on both the listener and the client extra-header path. Not TLS termination (ADR 0011).
