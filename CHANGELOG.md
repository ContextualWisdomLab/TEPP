...
- `compute_backend` VRAM budget types: 4/6/8/12/24-GiB profiles, safety reserve, peak prediction, micro-batch autotune, typed OOM as an expected operating state with bounded retry then CPU `f64` fallback, refusal of full-corpus device tensors and of dropping observations / shrinking complexity / moving a cutoff to fit memory, and a streamed weighted-sum CPU `f64` reference with computed RMSE (ADR 0006 first production slice; no live accelerator claim; no new migration).
