...
- `persistence_postgres` live-sqlx tenant-session affinity: retain one pool-backed PostgreSQL connection per live transport so tenant GUC binding and the following statement cannot be split across different pooled connections (ADR 0013).
