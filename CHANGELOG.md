...
- `identity_mapping` re-identification export: opaque analytical identifiers stay in ordinary compute; source-identity pairs export only under an explicit re-identification purpose; analytical purpose and blanket PII masking fail closed; recovered pairs match known truth at a higher computed rate than a collapsed map (ADR 0009).
