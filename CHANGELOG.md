...
- `membership_target` identity gate: language, episode, template, department, and opportunity-pool memberships cannot collapse into the entity/project pair stored by migration `0006`; recovered target kinds match known truth at a higher computed rate than treating every target as an entity (ADR 0003).
