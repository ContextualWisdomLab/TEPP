...
- `topic_measurement` logistic-normal additive log-ratio coordinates: fail-closed simplex validation, max-shifted stable ALR/inverse maps with true-parameter round-trip RMSE, explicit ALR-versus-ILR geometry boundary, and refusal of TF-IDF/BM25/keyword scores as inferential topic coordinates (ADR 0012 first production slice; no new migration).
