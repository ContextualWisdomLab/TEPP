...
- Irregular already-centered residual event-time log-rate in `psychometric_core`: caller-supplied within residuals with irregular `Δt` recover `a = ln(φ)/Δt`. Cluster-mean subtraction on a raw autoregressive series is not that estimand (Curran & Bauer, 2011, pp. 607–608). Still not DSEM.
