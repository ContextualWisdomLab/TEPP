...
- `concept_dictionary` language-profile validity gates: shared concept identity across native lexical channels, exact-span semantic units, refusal of translation-as-equivalence / default stopword deletion / TF-IDF-BM25 inferential weights, invariance-gated cross-language means, and CPU `f64` concept-coordinate RMSE (ADR 0004 first production slice; no new migration).
