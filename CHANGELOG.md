...
- `psychometric_core` posterior-aware ESEM/DSEM input gates: construct classification, refusal of raw-proportion Pearson, CPU `f64` OLS and plausible-value loading recovery with computed RMSE, invariance-gated latent-mean comparison, and refusal of causal language from temporal precedence, document linkage, event tracking, or prediction (ADR 0005 first production slice; no new migration).
