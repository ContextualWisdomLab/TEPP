...
- `irregular_time` event-time lags: consecutive lags are taken from event/valid time, equal system-time spacing cannot stand in for irregular event spacing, and recovered event lags match known truth with lower computed RMSE than an equal-spacing assumption.
