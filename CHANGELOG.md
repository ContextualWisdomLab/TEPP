...
- `privileged_access` audit replay: re-identification and grant-use decisions record purpose, opaque analytical subject, and allow/deny; source identity and blanket PII masking fail closed; replayed decisions match known truth at a higher computed rate than a collapsed allow-all log (ADR 0009).
