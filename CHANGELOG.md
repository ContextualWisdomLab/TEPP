...
- `persistence_postgres` entity/project target SQL: fail-closed insert/lookup for `entity_record` and `project_record` so membership foreign keys cannot be seeded from empty, oversized, or hostile type/status labels; interpolated codes are restricted to lowercase ASCII `snake_case` characters. No new migration number (ADR 0003 / ADR 0013).
- `persistence_postgres` live-sqlx tenant-session affinity and teardown: retain one pool-backed PostgreSQL connection per live transport so tenant GUC binding and the following statement cannot be split across different pooled connections, and close/drop the owned connection/runtime safely when called from another Tokio runtime (ADR 0013).
