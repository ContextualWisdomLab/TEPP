...
- `operational_log` source separation: `try_record` inspects source text, source identity, and blanket-mask intent before creating a line; `AnalyticalSubject` cannot be constructed from a source identity; privileged-export / identity-mapping / diagnosis action codes keep author, customer, and project memberships distinct; replayed lines match known truth at a higher computed rate than a collapsed single-action log (ADR 0009; ISO/IEC 29100:2024). The live docstring crate-root count is bound to `EXPECTED_CRATES` so the eleventh crate cannot fail a hard-coded `10`.
