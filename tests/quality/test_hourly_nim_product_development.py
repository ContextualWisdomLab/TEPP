...
class HourlyNimProductDevelopmentContractTests(unittest.TestCase):
    ...
    def test_supporting_runbook_and_doctoring_exist(self) -> None:
        ...
        self.assertIn("intentionally dispatch-only", runbook)
        self.assertIn("does not own a cron", runbook)
        self.assertIn("# cwl-org-commercial-entrypoint: v1", runbook)
        self.assertNotIn("minute 47 of every", runbook)
        self.assertNotIn('cron: "47 * * * *"', runbook)
